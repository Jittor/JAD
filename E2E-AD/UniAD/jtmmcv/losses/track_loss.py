...
from jtmmcv.models.dense_heads.track_head_plugin import Instances
from jtmmcv.core import build_assigner
from jtmmcv.models import build_loss
from jtmmcv.models.builder import LOSSES
from jtmmcv.core import reduce_mean
from jtmmcv.core.bbox.iou_calculators.iou3d_calculator import (
    bbox_overlaps_nearest_3d as iou_3d, )
from jtmmcv.core.bbox.util import denormalize_bbox
from jtmmcv.utils.general import SmoothL1Loss


def is_dist_avail_and_initialized():
    if jittor.in_mpi == False:
        return False
    return True
...
def accuracy(output, target, topk=(1, )):
    ...
    pred = pred.t()
    correct = pred.eq(target.view(1, -1).expand(pred.size))
...
class ClipMatcher(Module):
    def __init__(
        ...
        self.loss_predictions = SmoothL1Loss(reduction="none")
        self.register_buffer("code_weights",
                             jittor.array(code_weights).stop_grad())
    ...
    def _get_src_permutation_idx(self, indices):
        ...
        batch_idx = jittor.concat(
            [jittor.full_like(src, i) for i, (src, _) in enumerate(indices)])
        src_idx = jittor.concat([src for (src, _) in indices])
        return batch_idx, src_idx

    def _get_tgt_permutation_idx(self, indices):
        # permute targets following indices
        batch_idx = jittor.concat(
            [jittor.full_like(tgt, i) for i, (_, tgt) in enumerate(indices)])
        tgt_idx = jittor.concat([tgt for (_, tgt) in indices])
    ...
    def calc_loss_for_track_scores(self, track_instances: Instances):
        ...
        src_idx = jittor.arange(num_tracks, dtype=jittor.int32)
    ...
    def get_num_boxes(self, num_samples):
        num_boxes = jittor.array(num_samples,
                                    dtype=jittor.float32)
        if is_dist_avail_and_initialized():
            jittor.mpi.mpi_reduce(num_boxes)
        num_boxes = jittor.clamp(num_boxes / get_world_size(), min_v=1).item()
        return num_boxes
    ...
    def loss_cardinality(self, outputs, targets, indices):
        ...
        tgt_lengths = jittor.array([len(v.labels) for v in targets])
        # Count the number of predictions that are NOT "no-object" (which is the last class)
        card_pred = (pred_logits.argmax(-1)[0] !=
                     pred_logits.shape[-1] - 1).sum(1)
        card_err = jnn.l1_loss(card_pred.float(), tgt_lengths.float())
    ...
    def loss_past_trajs(self, outputs, gt_instances: List[Instances],
        ...
        target_trajs_mask = jittor.concat(
            [
    ...
    def compute_past_traj_loss(self, src, tgt, tgt_mask):
        loss = jittor.abs(src - tgt) * tgt_mask
        return jittor.sum(loss)/ (jittor.sum(tgt_mask>0) + 1e-5)
    ...
    def loss_boxes(self, outputs, gt_instances: List[Instances],
        ...
        for src_per_img, tgt_per_img in indices:
            keep = tgt_per_img != -1
            filtered_idx.append((src_per_img[keep.where()], tgt_per_img[keep.where()]))
        indices = filtered_idx
        idx = self._get_src_permutation_idx(indices)
        src_boxes = outputs["pred_boxes"][idx]
        sdc_boxes = outputs["pred_sdc_boxes"][0, -1:]
        target_sdc_boxes = gt_instances[0].sdc_boxes[:1]
        target_boxes = jittor.concat(
            [
                gt_per_img.boxes[i]
                for gt_per_img, (_, i) in zip(gt_instances, indices)
            ],
        ...
        src_boxes = jittor.concat([src_boxes, sdc_boxes], dim=0)
        target_boxes = jittor.concat([target_boxes, target_sdc_boxes], dim=0)
        ...
        target_obj_ids = jittor.concat([target_obj_ids, jittor.zeros(1)], dim=0)
        mask = target_obj_ids != -1
        bbox_weights = jittor.ones_like(target_boxes) * self.code_weights
        avg_factor = src_boxes[mask].size(0)
        avg_factor = reduce_mean(jittor.array([avg_factor]).to(dtype=target_boxes.dtype))
    ...
    def loss_labels(self,
        ...
        for gt_per_img, (_, J) in zip(gt_instances, indices):
            labels_per_img = jittor.ones_like(J) * self.num_classes
            # set labels of track-appear slots to num_classes
            if len(gt_per_img) > 0:
                labels_per_img[J != -1] = gt_per_img.labels[J[J != -1]]
            labels.append(labels_per_img)
        # [num_matched]
        target_classes_o = jittor.concat(labels)
        # [bs, num_query]
        target_classes[idx] = target_classes_o
        target_sdc_classes = gt_instances[0].sdc_labels[0:1].unsqueeze(0)
        if sdc_logits is not None:
            src_logits = jittor.concat([src_logits, sdc_logits], dim=1)
            target_classes = jittor.concat([target_classes, target_sdc_classes], dim=1)
        label_weights = jittor.ones_like(target_classes)
        # float tensor
        avg_factor = target_classes_o.numel(
        )  # pos + mathced gt for disapper track
        avg_factor += 1 # sdc
        
        avg_factor = reduce_mean(jittor.array([avg_factor]).to(dtype=src_logits.dtype))
    ...
    def match_for_single_frame(self,
        ...
        obj_idxes_list = obj_idxes.detach().numpy().tolist()
        obj_idx_to_gt_idx = {
            obj_idx: gt_idx
        ...
        full_track_idxes = jittor.arange(
            len(track_instances), dtype=jittor.int32)
        # previsouly tracked, which is matched by rule
        matched_track_idxes = track_instances.obj_idxes >= 0
        prev_matched_indices = jittor.stack(
            [
        ...
        tgt_state = jittor.zeros(len(gt_instances_i))
        tgt_state[tgt_indexes] = 1
        # new tgt indexes
        untracked_tgt_indexes = jittor.arange(len(gt_instances_i))[tgt_state == 0]
        ...
        def match_for_single_decoder_layer(unmatched_outputs, matcher):
            ...
            if isinstance(targets[0], Instances):
                ...
                gt_labels = jittor.concat(
                    [gt_per_img.labels for gt_per_img in targets])
                gt_bboxes = jittor.concat(
                    [gt_per_img.boxes for gt_per_img in targets])
            else:
                gt_labels = jittor.concat([v["labels"] for v in targets])
                gt_bboxes = jittor.concat([v["boxes"] for v in targets])
            ...
            if src_idx is None:
                return None
            # concat src and tgt.
            new_matched_indices = jittor.stack([
        ...
        if "aux_outputs" in outputs:
            for i, aux_outputs in enumerate(outputs["aux_outputs"]):
                ...
                matched_indices_layer = jittor.concat(
    ...
    def execute(self, outputs, input_data: dict):
    ...
    def prediction_loss(self, track_instances, predictions):
        ...
        for i in range(self._current_frame_idx, len(self.gt_instances)):
            ...
            if num_paired > 0:
                ...
                gt_boxes_i = jittor.concat([cx, cy, cz], dim=-1)
                ...
                self.losses_dict["pred_loss_{}".format(i)] = jittor.Var(
                    [0.0]).cuda()
