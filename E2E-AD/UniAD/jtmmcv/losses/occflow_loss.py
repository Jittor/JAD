...
from jtmmcv.models.losses.utils import weight_reduce_loss


def binary_cross_entropy_with_logits(logits, targets):
    """
    jittor实现等价于F.binary_cross_entropy_with_logits(logits, targets, reduction='none')
    """
    max_val = jittor.clamp(-logits, min_v=0)
...
class FieryBinarySegmentationLoss(Module):
    ...
    def execute(self, prediction, target, frame_mask=None):
        ...
        future_discounts = self.future_discount ** jittor.arange(s, dtype=loss.dtype)
        ...
        if self.use_top_k:
            ...
            loss, _ = jittor.sort(loss, dim=2, descending=True)
            loss = loss[:, :, :k]

        return self.loss_weight * jittor.mean(loss)
...
def dice_loss(pred,
    ...
    Args:
        pred (jittor.Var): The prediction, has a shape (n, *)
        target (jittor.Var): The learning label of the prediction,
    ...
    if naive_dice:
        b = jittor.sum(input, 1)
        c = jittor.sum(target, 1)
        d = (2 * a + eps) / (b + c + eps)
    else:
        b = jittor.sum(input * input, 1) + eps
        c = jittor.sum(target * target, 1) + eps
        d = (2 * a) / (b + c)
...
@LOSSES.register_module()
class DiceLossWithMasks(Module):
    def __init__(self,
                 use_sigmoid=True,
                 activate=True,
                 reduction='mean',
    ...
    def execute(self,
        ...
        Args:
            pred (jittor.Var): The prediction, has a shape (n, *).
            target (jittor.Var): The label of the prediction,
                shape (n, *), same shape of pred.
            weight (jittor.Var, optional): The weight of loss for each
                prediction, has a shape (n,). Defaults to None.
            avg_factor (int, optional): Average factor that is used to average
        ...
        Returns:
            jittor.Var: The calculated loss
        """
