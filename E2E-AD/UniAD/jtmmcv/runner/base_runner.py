...
class BaseRunner(metaclass=ABCMeta):
    """The base class of Runner, a training helper for jittor.
    ...
    Args:
        model (:obj:`jittor.nn.Module`): The model to be run.
        batch_processor (callable): A callable method that process a data
            batch. The interface of this method should be
            `batch_processor(model, data, train_mode) -> dict`
        optimizer (dict or :obj:`jittor.optim.Optimizer`): It can be either an
        ...
        if isinstance(optimizer, dict):
            for name, optim in optimizer.items():
                if not isinstance(optim, Optimizer):
                    raise TypeError(
                        f'optimizer must be a dict of jittor.optim.Optimizers, '
                        f'but optimizer["{name}"] is a {type(optim)}')
        elif not isinstance(optimizer, Optimizer) and optimizer is not None:
            raise TypeError(
                f'optimizer must be a jittor.optim.Optimizer object '
    ...
    def current_lr(self):
        ...
        """
        if isinstance(self.optimizer, jittor.optim.Optimizer):
    ...
    def current_momentum(self):
        ...
        elif isinstance(self.optimizer, jittor.optim.Optimizer):
    ...
    def resume(self,
        ...
        if 'optimizer' in checkpoint and resume_optimizer:
            if isinstance(self.optimizer, Optimizer):
                ...
                raise TypeError(
                    'Optimizer should be dict or jittor.optim.Optimizer '
