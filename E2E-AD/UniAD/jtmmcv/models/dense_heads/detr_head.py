...
from jtmmcv.models.bricks import Conv2d, Linear, build_activation_layer
from jtmmcv.models.bricks.transformer import FFN, build_positional_encoding
...
class DETRHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is DETRHead):
            ...
            class_weight = jittor.ones(num_classes + 1) * class_weight
    ...
    def _init_layers(self):
        ...
        self.input_proj = jnn.Conv2d(
            self.in_channels, self.embed_dims, kernel_size=1)
        self.fc_cls = jnn.Linear(self.embed_dims, self.cls_out_channels)
        self.reg_ffn = FFN(
            self.embed_dims,
            self.embed_dims,
            self.num_reg_fcs,
        ...
        self.query_embedding = jnn.Embedding(self.num_query, self.embed_dims)
    ...
    def execute(self, feats, img_metas):
        """Forward function.
    ...
    def forward_single(self, x, img_metas):
        ...
        masks = jnn.interpolate(
            masks.unsqueeze(1), size=x.shape[-2:]).to(bool).squeeze(1)
        # position encoding
    ...
    def simple_test_bboxes(self, feats, img_metas, rescale=False):
        ...
        Args:
            feats (tuple[jittor.Var]): Multi-level features from the
        ...
        outs = self.execute(feats, img_metas)
    ...
    def forward_single_onnx(self, x, img_metas):
        ...
        # The difference between jittor and exported ONNX model may be
    ...
    #     batch_index_offset = jittor.arange(batch_size).to(
    ...
    #     from jtmmcv.core.export import dynamic_clip_for_onnx
    #     x1, y1, x2, y2 = dynamic_clip_for_onnx(x1, y1, x2, y2, img_shape)
    #     det_bboxes = jittor.concat([x1, y1, x2, y2], dim=-1)
    #     det_bboxes = jittor.concat((det_bboxes, scores.unsqueeze(-1)), -1)
