...
from jtmmcv.models.utils.functional import (
...
class MotionHead(BaseMotionHead):
    ...
    def __init__(self,
        ...
        self.cls2group = jittor.array(self.cls2group)
    ...
    def forward_train(self,
        ...
        Args:
            ...
            gt_labels_3d (list[jittor.Var]): Labels of each sample.
            img_metas (list[dict]): Meta information of each sample.
            gt_fut_traj (list[jittor.Var]): Ground truth future trajectory of each sample.
            gt_fut_traj_mask (list[jittor.Var]): Ground truth future trajectory mask of each sample.
            gt_sdc_fut_traj (list[jittor.Var]): Ground truth future trajectory of each sample.
            gt_sdc_fut_traj_mask (list[jittor.Var]): Ground truth future trajectory mask of each sample.
            outs_track (dict): Outputs of track head.
            outs_seg (dict): Outputs of seg head.
            future_states (list[jittor.Var]): Ground truth future states of each sample.
        Returns:
        ...
        sdc_match_index = jittor.zeros((1,), dtype=all_matched_idxes[0].dtype)
        sdc_match_index[0] = gt_fut_traj[0].shape[0]
        all_matched_idxes = [jittor.concat([all_matched_idxes[0], sdc_match_index], dim=0)]
        gt_fut_traj[0] = jittor.concat([gt_fut_traj[0], gt_sdc_fut_traj[0]], dim=0)
        gt_fut_traj_mask[0] = jittor.concat([gt_fut_traj_mask[0], gt_sdc_fut_traj_mask[0]], dim=0)
        track_query = jittor.concat([track_query, outs_track['sdc_embedding'][None, None, None, :]], dim=2)
        sdc_track_boxes = outs_track['sdc_track_bbox_results']
        track_boxes[0][0].tensor = jittor.concat([track_boxes[0][0].tensor, sdc_track_boxes[0][0].tensor], dim=0)
        track_boxes[0][1] = jittor.concat([track_boxes[0][1], sdc_track_boxes[0][1]], dim=0)
        track_boxes[0][2] = jittor.concat([track_boxes[0][2], sdc_track_boxes[0][2]], dim=0)
        track_boxes[0][3] = jittor.concat([track_boxes[0][3], sdc_track_boxes[0][3]], dim=0)
        ...
        def filter_vehicle_query(outs_motion, all_matched_idxes, gt_labels_3d, vehicle_id_list):
            ...
            vehicle_mask = jittor.zeros_like(query_label)
    ...
    def forward_test(self, bev_embed, outs_track={}, outs_seg={}):
        ...
        def filter_vehicle_query(outs_motion, labels, vehicle_id_list):
            ...
            # select vehicle query according to vehicle_id_list
            vehicle_mask = jittor.zeros_like(labels)
            for veh_id in vehicle_id_list:
    ...
    def execute(self, 
        ...
        Args:
        bev_embed (jittor.Var): A tensor of shape (h*w, B, D) representing the bird's eye view embedding.
        track_query (jittor.Var): A tensor of shape (B, num_dec, A_track, D) representing the track query.
        lane_query (jittor.Var): A tensor of shape (N, M_thing, D) representing the lane query.
        lane_query_pos (jittor.Var): A tensor of shape (N, M_thing, D) representing the position of the lane query.
        track_bbox_results (List[jittor.Var]): A list of tensors containing the tracking bounding box results for each image in the batch.
        ...
        learnable_query_pos = jittor.stack(list(jittor.split(learnable_query_pos, self.num_anchor, dim=0)))
        ...
        agent_level_anchors = self.kmeans_anchors.to(dtype).view(num_groups, self.num_anchor, self.predict_steps, 2).detach()
        ...
        inter_states, inter_references = self.motionformer(
            ...
            spatial_shapes=jittor.array(
                [[self.bev_h, self.bev_w]]),
            level_start_index=jittor.array([0]))
        ...
        for lvl in range(inter_states.shape[0]):
            ...
            tmp[..., :2] = jittor.cumsum(tmp[..., :2], dim=3)
        ...
        outputs_traj_scores = jittor.stack(outputs_traj_scores)
    ...
    def group_mode_query_pos(self, bbox_results, mode_query_pos):
        ...
        Args:
            ...
            mode_query_pos (jittor.Var): A tensor of shape (B, A, G, P, D) representing the mode query positions.
        
        Returns:
            jittor.Var: A tensor of shape (B, A, P, D) representing the classified mode query positions.
        """
        ...
        for i in range(batch_size):
            ...
            batched_mode_query_pos.append(jittor.stack(grouped_mode_query_pos,dim=1).squeeze(0))
        return jittor.stack(batched_mode_query_pos)
    ...
    def loss(self,
        ...
        Args:
            gt_bboxes_3d (List[jittor.Var]): A list of tensors representing ground truth 3D bounding boxes for each image.
            gt_fut_traj (jittor.Var): A tensor representing the ground truth future trajectories.
            gt_fut_traj_mask (jittor.Var): A tensor representing the ground truth future trajectory masks.
            preds_dicts_motion (Dict[str, jittor.Var]): A dictionary containing motion-related prediction tensors.
            all_matched_idxes (List[jittor.Var]): A list of tensors containing the matched ground truth indices for each image in the batch.
            track_bbox_results (List[Tuple[jittor.Var]]): A list of tuples containing the tracking bounding box results for each image in the batch.
    ...
    def compute_matched_gt_traj(self,
        ...
        Args:
        gt_fut_traj (jittor.Var): Ground truth future trajectories of shape (num_imgs, num_objects, num_future_steps, 2).
        gt_fut_traj_mask (jittor.Var): Ground truth future trajectory masks of shape (num_imgs, num_objects, num_future_steps, 2).
        all_matched_idxes (List[jittor.Var]): A list of tensors containing the matched indexes for each image in the batch.
        track_bbox_results (List[jittor.Var]): A list of tensors containing the tracking bounding box results for each image in the batch.
        gt_bboxes_3d (List[jittor.Var]): A list of tensors containing the ground truth 3D bounding boxes for each image in the batch.

        Returns:
        jittor.Var: A concatenated tensor of the matched ground truth future trajectories.
        jittor.Var: A concatenated tensor of the matched ground truth future trajectory masks.
        """
        ...
        for i in range(num_imgs):
            ...
            if self.use_nonlinear_optimizer:
                ...
                matched_gt_fut_traj = jittor.concat(
                    [matched_gt_fut_traj, sdc_gt_fut_traj], dim=0)
                matched_gt_fut_traj_mask = jittor.concat(
                    [matched_gt_fut_traj_mask, sdc_gt_fut_traj_mask], dim=0)
            matched_gt_fut_traj_mask = jittor.all(
        ...
        gt_fut_traj_all = jittor.concat(gt_fut_traj_all, dim=0)
        gt_fut_traj_mask_all = jittor.concat(gt_fut_traj_mask_all, dim=0)
    ...
    def compute_loss_traj(self,
        ...
        Args:
            traj_scores (jittor.Var): A tensor representing the trajectory scores.
            traj_preds (jittor.Var): A tensor representing the predicted trajectories.
            gt_fut_traj_all (jittor.Var): A tensor representing the ground truth future trajectories.
            gt_fut_traj_mask_all (jittor.Var): A tensor representing the ground truth future trajectory masks.
            all_matched_idxes (List[jittor.Var]): A list of tensors containing the matched ground truth indices for each image in the batch.
        ...
        traj_prob_all = jittor.concat(traj_prob_all, dim=0)
        traj_preds_all = jittor.concat(traj_preds_all, dim=0)
