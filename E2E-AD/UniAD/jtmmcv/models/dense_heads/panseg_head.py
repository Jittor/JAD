...
from jtmmcv.models.utils import bias_init_with_prob, constant_init
...
from jtmmcv.core.bbox.builder import build_assigner, build_sampler
from jtmmcv.core.utils import multi_apply, reduce_mean
from jtmmcv.models.utils import build_transformer
...
class PansegformerHead(SegDETRHead):
    ...
    def _init_layers(self):
        ...
        if not self.as_two_stage:
            self.bev_embedding = jnn.Embedding(self.bev_h * self.bev_w, self.embed_dims)
        ...
        if self.with_box_refine:
            self.cls_branches = jnn.ModuleList(
                [Linear(self.embed_dims, self.cls_out_channels) for _ in range(num_pred)])
            ...
            for _ in range(num_pred):
                ...
                layers.append(Linear(self.embed_dims, 4))
        ...
        if not self.as_two_stage:
            self.query_embedding = jnn.Embedding(self.num_query,
                                                self.embed_dims * 2)
        self.stuff_query = jnn.Embedding(self.num_stuff_classes,
                                        self.embed_dims * 2)

        self.reg_branches2 = jnn.ModuleList()
        for _ in range(self.num_dec_things):
            ...
            for _ in range(self.num_reg_fcs):
                ...
                layers.append(jnn.ReLU())
            layers.append(Linear(self.embed_dims, 4))
            self.reg_branches2.append(jnn.Sequential(*layers))
        self.cls_thing_branches = jnn.ModuleList(
            [Linear(self.embed_dims, self.cls_out_channels) for _ in range(self.num_dec_things)])
        self.cls_stuff_branches = jnn.ModuleList(
            [Linear(self.embed_dims, 1) for _ in range(self.num_dec_stuff)])
    ...
    def init_weights(self):
        ...
        if self.loss_cls.use_sigmoid:
            ...
            for m in self.cls_branches:
                jnn.init.constant_(m.bias, bias_init)
        ...
        for m in self.reg_branches2:
            constant_init(m[-1], 0, bias=0)
        jnn.init.constant_(self.reg_branches[0][-1].bias[2:], -2.0)

        if self.as_two_stage:
            for m in self.reg_branches:
                jnn.init.constant_(m[-1].bias[2:], 0.0)

    def execute(self, bev_embed):
        """Forward function.
        ...
        mlvl_feats = [jittor.reshape(bev_embed, (bs, self.bev_h, self.bev_w ,-1)).permute(0, 3, 1, 2)]
        ...
        for feat in mlvl_feats:
            mlvl_masks.append(
                jnn.interpolate(img_masks[None],
                              size=feat.shape[-2:]).to(bool).squeeze(0))
    ...
    def loss(
        ...
        if enc_cls_scores is not None:
            binary_labels_list = [
                jittor.zeros_like(gt_things_lables_list[i])
    ...
    def _filter_query_single(self,
        ...
        # bbox targets
        bbox_targets = jittor.zeros_like(bbox_pred)
        ...
        factor = bbox_pred.new([img_w, img_h, img_w,
                                       img_h]).unsqueeze(0)
    ...
    def get_filter_results_and_loss(self, cls_scores, bbox_preds,
        ...
        num_total_pos_thing = loss_cls.new(num_total_pos_thing)
        num_total_pos_thing = jittor.clamp(reduce_mean(num_total_pos_thing),
                                          min_v=1).mean()
        ...
        factors = jittor.concat(factors, 0)
    ...
    def loss_single_panoptic(self,
        ...
        thing_query = jittor.zeros([BS, len_query, dim_query])

        stuff_query, stuff_query_pos = jittor.split(self.stuff_query.weight,
                                                   self.embed_dims,
                                                   dim=1)
        ...
        mask_inter_things = jittor.stack(mask_inter_things, 0).squeeze(-1)

        mask_stuff = mask_stuff.squeeze(-1)
        mask_inter_stuff = jittor.stack(mask_inter_stuff, 0).squeeze(-1)
        ...
        for i in range(BS):
            ...
            for j in range(self.num_dec_things):
                ...
                if len(pos_ind) == 0:
                    tmp = tmp.sum(
                    ) + reference_i  # for reply bug of jittor broadcast
                ...
                new_bbox_preds[j][i][:len(pos_inds_mask_list[i])] = outputs_coord.reshape(-1, 4)
        ...
        mask_preds_things = jittor.concat(mask_preds_things, 0)
        mask_preds_inter_things = [
            jittor.concat(each, 0) for each in mask_preds_inter_things
        ]
        cls_thing_preds = [jittor.concat(each, 0) for each in cls_thing_preds]
        cls_stuff_preds = [jittor.concat(each, 0) for each in cls_stuff_preds]
        mask_preds_stuff = jittor.concat(mask_preds_stuff, 0)
        mask_preds_inter_stuff = [
            jittor.concat(each, 0) for each in mask_preds_inter_stuff
        ]
        cls_scores_list = [
        ...
        thing_labels = jittor.concat(labels_list, 0)
        things_weights = jittor.concat(label_weights_list, 0)

        bboxes_taget = jittor.concat(bbox_targets_list)
        bboxes_weights = jittor.concat(bbox_weights_list)
        ...
        mask_things_gt = jittor.concat(mask_targets_list, 0).to(jittor.float32)

        mask_weight_things = jittor.concat(mask_weights_list,
                                       0)
        mask_stuff_gt = []
        mask_weight_stuff = []
        stuff_labels = []
        ...
        for i in range(BS):
            ...
            mask_weight_i_stuff = jittor.zeros([self.num_stuff_classes])
            mask_weight_i_stuff[select_stuff_index] = 1
            stuff_masks = jittor.zeros(
                (self.num_stuff_classes, *mask_targets_list[i].shape[-2:])).to(bool)
            stuff_masks[select_stuff_index] = gt_stuff_masks_list[i].to(
                bool)
            mask_stuff_gt.append(stuff_masks)
            select_stuff_index = jittor.concat([
                select_stuff_index,
                jittor.array((self.num_stuff_classes))
            ])

            stuff_labels.append(1 - mask_weight_i_stuff)
            mask_weight_stuff.append(mask_weight_i_stuff)

        mask_weight_stuff = jittor.concat(mask_weight_stuff,
                                      0)
        stuff_labels = jittor.concat(stuff_labels, 0)
        mask_stuff_gt = jittor.concat(mask_stuff_gt, 0).to(jittor.float32)

        num_total_pos_stuff = loss_cls.new(num_total_pos_stuff)
        num_total_pos_stuff = jittor.clamp(reduce_mean(num_total_pos_stuff),
                                          min_v=1).item()
        if mask_preds_things.shape[0] == 0:
            loss_mask_things = (0 * mask_preds_things).sum()
        else:
            mask_preds = jnn.interpolate(mask_preds_things.unsqueeze(0),
                                       scale_factor=2.0,
                                       mode='bilinear').squeeze(0)
            mask_targets_things = jnn.interpolate(mask_things_gt.unsqueeze(0),
        ...
        if mask_preds_stuff.shape[0] == 0:
            ...
            mask_preds = jnn.interpolate(mask_preds_stuff.unsqueeze(0),
                                       scale_factor=2.0,
                                       mode='bilinear').squeeze(0)
            mask_targets_stuff = jnn.interpolate(mask_stuff_gt.unsqueeze(0),
                                               size=mask_preds.shape[-2:],
                                               mode='bilinear').squeeze(0)
        ...
        for j in range(len(mask_preds_inter_things)):
            ...
            if mask_preds_this_level.shape[0] == 0:
                ...
                mask_preds_this_level = jnn.interpolate(
    ...
    def forward_test(self,
        ...
        with jittor.no_grad():
            ...
            lanes_pred = (results[0]['lane'].sum(0) > 0).to(jittor.int32).int()
            lanes_gt = (gt_lane_masks[0][0][:-1].to(jittor.int32).sum(0) > 0).to(jittor.int32).int()
            lanes_iou, lanes_intersection, lanes_union = IOU(lanes_pred.view(1, -1), lanes_gt.view(1, -1))

            
            strange = (gt_lane_masks[0][0].to(jittor.int32))[gt_lane_labels[0][0] == 0].to(jittor.int32).sum(dim=0) > 0
            divider_gt = (strange).to(jittor.int32).int()
            crossing_gt = (gt_lane_masks[0][0][gt_lane_labels[0][0] == 1].to(jittor.int32).sum(0) > 0).to(jittor.int32).int()
            contour_gt = (gt_lane_masks[0][0][gt_lane_labels[0][0] == 2].to(jittor.int32).sum(0) > 0).to(jittor.int32).int()
    ...
    def forward_train(self,
        ...
        Args:
            bev_feat (jittor.Var): Bird's eye view feature maps. Shape [batch_size, channels, height, width].
            img_metas (list[dict]): List of image meta information dictionaries.
            gt_lane_labels (list[jittor.Var]): Ground-truth lane class labels. Shape [batch_size, num_lanes, max_lanes].
            gt_lane_bboxes (list[jittor.Var]): Ground-truth lane bounding boxes. Shape [batch_size, num_lanes, 4].
            gt_lane_masks (list[jittor.Var]): Ground-truth lane masks. Shape [batch_size, num_lanes, height, width].
            prev_bev (jittor.Var): Previous bird's eye view feature map. Shape [batch_size, channels, height, width].

        Returns:
            tuple:
                - losses_seg (jittor.Var): Total segmentation loss.
    ...
    def _get_bboxes_single(self,
        ...
        if self.loss_cls.use_sigmoid:
            ...
            det_labels, scores = jnn.softmax(cls_score, dim=-1)[..., :-1].argmax(-1)
        ...
        det_bboxes[:, 0::2].clamp_(min_v=0, max_v=img_shape[1])
        det_bboxes[:, 1::2].clamp_(min_v=0, max_v=img_shape[0])
        if rescale:
            det_bboxes /= det_bboxes.new(scale_factor)
        det_bboxes = jittor.concat((det_bboxes, scores.unsqueeze(1)), -1)
    ...
    def get_bboxes(
        ...
        for img_id in range(len(img_metas)):
            ...
            attn_map = jittor.concat([mask_things, mask_stuff], 1)
            ...
            mask_pred = jnn.interpolate(mask_pred.unsqueeze(0),
                                      size=ori_shape[:2],
                                      mode='bilinear').squeeze(0)
            ...
            # scores_all = jittor.concat([bbox[:, -1], scores_stuff], 0)
            # bboxes_all = jittor.concat([bbox, jittor.zeros([self.num_stuff_classes, 5], device=labels.device)], 0)
            # labels_all = jittor.concat([labels, jittor.arange(self.num_things_classes, self.num_things_classes+self.num_stuff_classes).to(labels.device)], 0)
            scores_all = bbox[:, -1]
            bboxes_all = bbox
            labels_all = labels
            ...
            scores_all, index = jittor.sort(scores_all, descending=True)
            ...
            results = jittor.zeros((2, *mask_pred.shape[-2:]),
                                 ).to(jittor.int32)
            id_unique = 1
            lane = jittor.zeros((self.num_things_classes, *mask_pred.shape[-2:])).to(jittor.int32)
            lane_score =  jittor.zeros((self.num_things_classes, *mask_pred.shape[-2:])).to(mask_pred.dtype)
            ...
            panoptic_list.append(
                (results.permute(1, 2, 0).numpy(), file_name, ori_shape))
