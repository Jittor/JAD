...
def nonlinear_smoother(gt_bboxes_3d, gt_fut_traj, gt_fut_traj_mask, bbox_tensor):
    ...
    Args:
    gt_bboxes_3d (jittor.Var): Ground truth 3D bounding boxes of shape (batch_size, 7).
    gt_fut_traj (jittor.Var): Ground truth future trajectories of shape (batch_size, 12, 2).
    gt_fut_traj_mask (jittor.Var): A mask indicating valid timesteps in the ground truth future trajectories of shape (batch_size, 12).
    bbox_tensor (jittor.Var): A tensor representing the bounding box properties of shape (batch_size, 9).

    Returns:
    jittor.Var: The perturbed trajectories of shape (batch_size, 12, 2).
    jittor.Var: The updated mask indicating valid timesteps in the perturbed trajectories of the same shape as gt_fut_traj_mask.
    """
    dtype = gt_fut_traj.dtype
    gt_bboxes_3d = gt_bboxes_3d.detach().numpy()
    gt_fut_traj = gt_fut_traj.detach().numpy()
    ...
    gt_fut_traj_mask = gt_fut_traj_mask.detach().numpy()
    bbox_tensor = bbox_tensor.detach().numpy()
    ...
    return jittor.array(traj_perturb_all, dtype=dtype), jittor.array(gt_fut_traj_mask > 0)
