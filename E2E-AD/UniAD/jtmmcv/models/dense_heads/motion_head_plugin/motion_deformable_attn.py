...
from jtmmcv.ops.ms_deform_att import multi_scale_deformable_attn_jittor, MultiScaleDeformableAttnFunction
from jtmmcv.models.utils import xavier_init, constant_init
from jtmmcv.models.bricks.registry import ATTENTION, TRANSFORMER_LAYER
from jtmmcv.models.bricks.transformer import build_attention, build_feedforward_network, build_norm_layer, MultiheadAttention
from jtmmcv.models.bricks.drop import build_dropout
from jtmmcv.models.backbones.base_module import BaseModule, ModuleList, Sequential
from jtmmcv.utils import ConfigDict, deprecated_api_warning
...
class MotionDeformableAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.norm_cfg = norm_cfg
        self.dropout = jnn.Dropout(dropout)
        self.batch_first = batch_first
        ...
        self.sampling_offsets = jnn.Linear(
            embed_dims, num_heads * num_steps * num_levels * num_points * 2)
        self.attention_weights = jnn.Linear(embed_dims,
                                           num_heads * num_steps * num_levels * num_points)
        self.value_proj = jnn.Linear(embed_dims, embed_dims)
        self.output_proj = Sequential(jnn.Linear(num_steps*embed_dims, embed_dims),
    ...
    def init_weights(self):
        ...
        thetas = jittor.arange(
            self.num_heads,
            dtype=jittor.float32) * (2.0 * math.pi / self.num_heads)
        grid_init = jittor.stack([thetas.cos(), thetas.sin()], -1)
        grid_init = (grid_init /
    ...
    def execute(self,
        ...
        query = jittor.flatten(query, start_dim=1, end_dim=2)
        ...
        if reference_trajs.shape[-1] == 2:
            ...
            reference_trajs_shape = reference_trajs.shape
            if self.sample_index < 0:
                positive_index = reference_trajs_shape[3] + self.sample_index
            else:
                positive_index = self.sample_index   
            
            reference_trajs = reference_trajs[:, :, :, [positive_index], :, :] #jittor does not support negative [index]
            reference_trajs_ego = self.agent_coords_to_ego_coords(copy.deepcopy(reference_trajs), bbox_results).detach()
            reference_trajs_ego = jittor.flatten(reference_trajs_ego, start_dim=1, end_dim=2)
            ...
            offset_normalizer = jittor.stack(
                [spatial_shapes[..., 1], spatial_shapes[..., 0]], -1)
            sampling_locations = reference_trajs_ego \
                + sampling_offsets \
                / offset_normalizer[None, None, None, None, :, None, :]

            sampling_locations = jittor.einops.rearrange(sampling_locations, 'bs nq nh ns nl np c -> bs nq ns nh nl np c') # permute([0,1,3,2,4,5,6])
            attention_weights = jittor.einops.rearrange(attention_weights, 'bs nq nh ns nl np -> bs nq ns nh nl np') #.permute([0,1,3,2,4,5])
        ...
        output = jittor.flatten(output, start_dim=2, end_dim=3)
    ...
    def agent_coords_to_ego_coords(self, reference_trajs, bbox_results):
        ...
        return jittor.stack(reference_trajs_ego)
...
class CustomModeMultiheadAttention(BaseModule):
    """A wrapper for ``jittor.nn.MultiheadAttention``.
    ...
    def __init__(self,
        ...
        self.dropout_layer = build_dropout(
            dropout_layer) if dropout_layer else jnn.Identity()
    ...
    def execute(self,
        ...
        **kwargs allow passing a more general data flow when combining
        ...
        Args:
            ...
            value (Tensor): The value tensor with same shape as `key`.
                Same in `nn.MultiheadAttention.execute`. Defaults to None.
            ...
            attn_mask (Tensor): ByteTensor mask with shape [num_queries,
                num_keys]. Same in `nn.MultiheadAttention.execute`.
                Defaults to None.
        ...
        # ``jittor.nn.MultiheadAttention`` is (num_query, batch,
        # embed_dims), We should adjust the shape of dataflow from
        # batch_first (batch, num_query, embed_dims) to num_query_first
        # (num_query ,batch, embed_dims), and recover ``attn_output``
        # from num_query_first to batch_first.
        query = jittor.flatten(query, start_dim=0, end_dim=1)
        key = jittor.flatten(key, start_dim=0, end_dim=1)
        value = jittor.flatten(value, start_dim=0, end_dim=1)
        identity = jittor.flatten(identity, start_dim=0, end_dim=1)
