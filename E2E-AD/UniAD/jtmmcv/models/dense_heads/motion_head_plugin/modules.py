...
from jtmmcv.models.bricks.registry import TRANSFORMER_LAYER_SEQUENCE
from jtmmcv.models.bricks.transformer import build_transformer_layer
...
from jtmmcv.models.bricks.transformer import TransformerDecoderLayer
...
class MotionTransformerDecoder(BaseModule):
    ...
    def __init__(self, pc_range=None, embed_dims=256, transformerlayers=None, num_layers=3, **kwargs):
        ...
        self.track_agent_interaction_layers = jnn.ModuleList(
            [TrackAgentInteraction() for i in range(self.num_layers)])
        self.map_interaction_layers = jnn.ModuleList(
            [MapInteraction() for i in range(self.num_layers)])
        self.bev_interaction_layers = jnn.ModuleList(
            [build_transformer_layer(transformerlayers) for i in range(self.num_layers)])

        self.static_dynamic_fuser = jnn.Sequential(
            jnn.Linear(self.embed_dims*2, self.embed_dims*2),
            jnn.ReLU(),
            jnn.Linear(self.embed_dims*2, self.embed_dims),
        )
        self.dynamic_embed_fuser = jnn.Sequential(
            jnn.Linear(self.embed_dims*3, self.embed_dims*2),
            jnn.ReLU(),
            jnn.Linear(self.embed_dims*2, self.embed_dims),
        )
        self.in_query_fuser = jnn.Sequential(
            jnn.Linear(self.embed_dims*2, self.embed_dims*2),
            jnn.ReLU(),
            jnn.Linear(self.embed_dims*2, self.embed_dims),
        )
        self.out_query_fuser = jnn.Sequential(
            jnn.Linear(self.embed_dims*4, self.embed_dims*2),
            jnn.ReLU(),
            jnn.Linear(self.embed_dims*2, self.embed_dims),
        )
    ...
    def execute(self,
        ...
        query_embed = jittor.zeros_like(static_intention_embed)
        for lid in range(self.num_layers):
            ...
            dynamic_query_embed = self.dynamic_embed_fuser(jittor.concat(
                [agent_level_embedding, scene_level_offset_embedding, scene_level_ego_embedding], dim=-1))
            
            # fuse static and dynamic intention embedding
            query_embed_intention = self.static_dynamic_fuser(jittor.concat(
                [static_intention_embed, dynamic_query_embed], dim=-1))  # (B, A, P, D)
            
            # fuse intention embedding with query embedding
            query_embed = self.in_query_fuser(jittor.concat([query_embed, query_embed_intention], dim=-1))
            ...
            query_embed = jittor.concat(query_embed, dim=-1)
            ...
            if traj_reg_branches is not None:
                ...
                new_reference_trajs = jittor.zeros_like(reference_trajs)
        ...
        return jittor.stack(intermediate), jittor.stack(intermediate_reference_trajs)
...
class TrackAgentInteraction(BaseModule):
    ...
    def __init__(self,
        ...
        self.interaction_transformer = TransformerDecoderLayer(d_model=embed_dims,
                                                                  nhead=num_heads,
                                                                  dropout=dropout,
                                                                  dim_feedexecute=embed_dims*2,
                                                                  batch_first=batch_first)

    def execute(self, query, key, query_pos=None, key_pos=None):
        '''
...
class IntentionInteraction(BaseModule):
    ...
    def __init__(self,
        ...
        self.batch_first = batch_first
        from jtmmcv.models.bricks.transformer import TransformerEncoderLayer
        self.interaction_transformer = TransformerEncoderLayer(d_model=embed_dims,
                                                                  nhead=num_heads,
                                                                  dropout=dropout,
                                                                  dim_feedexecute=embed_dims*2,
                                                                  batch_first=batch_first)
    ...
    def execute(self, query):
        B, A, P, D = query.shape
        # B, A, P, D -> B*A,P, D
        rebatch_x = jittor.flatten(query, start_dim=0, end_dim=1)
