...
from jtmmcv.core.utils import reduce_mean
...
def uniform(shape, dtype="float32", low=0, high=1):
    ...
    '''
    return jittor.random(jittor.NanoVector(shape), dtype) * (low - high) + high


def xavier_normal(shape, dtype="float32", gain=1.0):
    ...
    bound = gain * math.sqrt(2.0/fan)
    return uniform(shape, dtype, -bound, bound)

def xavier_normal_(var, gain=1.0):
    ''' Inplace initialize Jittor Var by xavier_uniform.
    The resulting var will have values sampled from
    :math:`uniform(-a, a)` where
    ...
    '''
    return var.assign(xavier_normal(tuple(var.shape), var.dtype, gain))
...
class OccHead(BaseModule):
    def __init__(self, 
        ...
        if bev_proj_nlayers == 0:
            self.bev_light_proj = jnn.Sequential()
        else:
        ...
        self.base_downscale = jnn.Sequential(
        ...
        self.temporal_mlps = jnn.ModuleList([
            MLP(query_dim, query_dim, bev_proj_dim, num_layers=temporal_mlp_layer)
            for _ in range(self.n_future_blocks)
        ])
        
        self.downscale_convs = jnn.ModuleList([
            Bottleneck(in_channels=bev_proj_dim, downsample=True)
            for _ in range(self.n_future_blocks)
        ])
        
        self.upsample_adds = jnn.ModuleList([
            UpsamplingAdd(in_channels=bev_proj_dim, out_channels=bev_proj_dim)
            for _ in range(self.n_future_blocks)
        ])
        ...
        # Query
        self.mode_fuser = jnn.Sequential(
                jnn.Linear(query_dim, bev_proj_dim),
                jnn.LayerNorm(bev_proj_dim),
                jnn.ReLU()
            )
        self.multi_query_fuser =  jnn.Sequential(
                jnn.Linear(query_dim * 3, query_dim * 2),
                jnn.LayerNorm(query_dim * 2),
                jnn.ReLU(),
                jnn.Linear(query_dim * 2, bev_proj_dim),
            )
        ...
        self.temporal_mlp_for_mask = MLP(
            query_dim, query_dim, bev_proj_dim, num_layers=query_mlp_layers
        )
    ...
    def get_attn_mask(self, state, ins_query):
        ...
        mask_pred = jittor.linalg.einsum("bqc,bchw->bqhw", ins_embed, state)
        ...
        attn_mask[jittor.where(
            attn_mask.sum(-1) == attn_mask.shape[-1])] = False

        upsampled_mask_pred = jnn.interpolate(
            mask_pred,
            self.bev_size,
            mode='bilinear',
            align_corners=False
    ...
    def execute(self, x, ins_query):
        ...
        future_states = jittor.stack(future_states, dim=1)  # [b, t, d, h/4, w/4]
        temporal_query = jittor.stack(temporal_query, dim=1)  # [b, t, q, d]
        mask_preds = jittor.stack(mask_preds, dim=2)  # [b, q, t, h, w]
        ins_query = jittor.stack(temporal_embed_for_mask_attn, dim=1)  # [b, t, q, d]
        ...
        ins_occ_logits = jittor.linalg.einsum("btqc,btchw->bqthw", ins_occ_query, future_states)
    ...
    def merge_queries(self, outs_dict, detach_query_pos=True):
        ...
        ins_query = self.mode_fuser(ins_query).argmax(2)[1]
        ins_query = self.multi_query_fuser(jittor.concat([ins_query, track_query, track_query_pos], dim=-1))
    ...
    def forward_train(
        ...
        future_frame_mask.data[~past_valid.data] = False
        ...
        for ind in range(bs):
            ...
            for ins_id in cur_gt_inds:
                ...
                if (ins_seg_targets == self.ignore_index).all().item() is True:
                    ...
                    ins_tgt = jittor.ones_like(ins_seg_targets).long() * self.ignore_index
                else:
                    SPECIAL_INDEX = -20
                    if ins_id.item() == self.ignore_index:
                        ins_id = jittor.ones_like(ins_id) * SPECIAL_INDEX
                    ins_tgt = (ins_seg_targets == ins_id).long()  # [t, h, w], 0 or 1
                
                ins_seg_targets_ordered.append(ins_tgt)
            
            ins_seg_targets_ordered = jittor.stack(ins_seg_targets_ordered, dim=0)  # [n_gt, t, h, w]
            ...
            num_total_pos = ins_seg_preds.new(num_total_pos)
            num_total_pos = jittor.clamp(reduce_mean(num_total_pos), min_v=1).tolist()
    ...
    def forward_test(
        ...
        if no_query:
            ...
            out_dict['seg_out'] = jittor.zeros_like(out_dict['seg_gt']).long()  # [1, 5, 1, 200, 200]
            out_dict['ins_seg_out'] = jittor.zeros_like(out_dict['ins_seg_gt']).long()  # [1, 5, 200, 200]
            return out_dict
        ...
        pred_seg_scores = pred_ins_sigmoid.argmax(1)[1]
    ...
    def get_ins_seg_gt(self, gt_instance):
        ins_gt_old = gt_instance.contiguous()  # Not consecutive, 0 for bg, otherwise ins_ind(start from 1)
        ins_gt_new = jittor.zeros_like(ins_gt_old).to(ins_gt_old)  # Make it consecutive
        ins_inds_unique = jittor.unique(ins_gt_old)
        new_id = 1
