...
from jtmmcv.models.bricks.attn import MultiheadAttention
...
class MemoryBank(Module):
    ...
    def _build_layers(self, args, dim_in, hidden_dim, dim_out):
        ...
        self.save_proj = jnn.Linear(dim_in, dim_in)

        self.temporal_attn = MultiheadAttention(dim_in, 8, dropout=0)
        self.temporal_fc1 = jnn.Linear(dim_in, hidden_dim)
        self.temporal_fc2 = jnn.Linear(hidden_dim, dim_in)
        self.temporal_norm1 = jnn.LayerNorm(dim_in)
        self.temporal_norm2 = jnn.LayerNorm(dim_in)
    ...
    def update(self, track_instances):
        ...
        if len(saved_embed) > 0:
            ...
            mem_padding_mask[saved_idxes] = jittor.concat([mem_padding_mask[saved_idxes, 1:], jittor.zeros((len(saved_embed), 1), dtype=bool)], dim=1)
            track_instances.mem_bank = track_instances.mem_bank.clone()
            track_instances.mem_bank[saved_idxes] = jittor.concat([prev_embed[:, 1:], save_embed], dim=1)
    ...
    def _forward_temporal_attn(self, track_instances):
        ...
        if len(embed) > 0:
            ...
            embed2 = self.temporal_fc2(jnn.relu(self.temporal_fc1(embed)))
    ...
    def execute(self, track_instances: Instances, update_bank=True) -> Instances:
...
# QIM
class QueryInteractionBase(Module):
...
class QueryInteractionModule(QueryInteractionBase):
    ...
    def _build_layers(self, args, dim_in, hidden_dim, dim_out):
        ...
        self.self_attn = MultiheadAttention(dim_in, 8, dropout=dropout)
        self.linear1 = jnn.Linear(dim_in, hidden_dim)
        self.dropout = jnn.Dropout(dropout)
        self.linear2 = jnn.Linear(hidden_dim, dim_in)

        if args["update_query_pos"]:
            self.linear_pos1 = jnn.Linear(dim_in, hidden_dim)
            self.linear_pos2 = jnn.Linear(hidden_dim, dim_in)
            self.dropout_pos1 = jnn.Dropout(dropout)
            self.dropout_pos2 = jnn.Dropout(dropout)
            self.norm_pos = jnn.LayerNorm(dim_in)

        self.linear_feat1 = jnn.Linear(dim_in, hidden_dim)
        self.linear_feat2 = jnn.Linear(hidden_dim, dim_in)
        self.dropout_feat1 = jnn.Dropout(dropout)
        self.dropout_feat2 = jnn.Dropout(dropout)
        self.norm_feat = jnn.LayerNorm(dim_in)

        self.norm1 = jnn.LayerNorm(dim_in)
        self.norm2 = jnn.LayerNorm(dim_in)

        self.dropout1 = jnn.Dropout(dropout)
        self.dropout2 = jnn.Dropout(dropout)
    ...
    def _random_drop_tracks(self, track_instances: Instances) -> Instances:
        ...
        if drop_probability > 0 and len(track_instances) > 0:
            keep_idxes = jittor.rand_like(track_instances.scores) > drop_probability
    ...
    def _add_fp_tracks(self, track_instances: Instances,
        ...
        if len(inactive_instances) > 0 and num_fp > 0:
            if num_fp >= len(inactive_instances):
                ...
                fp_indexes = jittor.argsort(inactive_instances.scores)[-num_fp:][0]
                fp_track_instances = inactive_instances[fp_indexes]
    ...
    def execute(self, data) -> Instances:
