...
from jtmmcv.metrics.classification import stat_scores_multiple_classes
from jtmmcv.metrics.reduction import reduce
...
class IntersectionOverUnion(Metric):
    ...
    def __init__(
        ...
        self.add_state('true_positive', default=jittor.zeros(n_classes), dist_reduce_fx='sum')
        self.add_state('false_positive', default=jittor.zeros(n_classes), dist_reduce_fx='sum')
        self.add_state('false_negative', default=jittor.zeros(n_classes), dist_reduce_fx='sum')
        self.add_state('support', default=jittor.zeros(n_classes), dist_reduce_fx='sum')

    def update(self, prediction: jittor.Var, target: jittor.Var):
    ...
    def compute(self):
        scores = jittor.zeros(self.n_classes, dtype=jittor.float32)
        ...
        for class_idx in range(self.n_classes):
            ...
            score = tp.to(jittor.float32) / denominator
            scores[class_idx] = score

        # Remove the ignored class index from the scores.
        if (self.ignore_index is not None) and (0 <= self.ignore_index < self.n_classes):
            scores = jittor.concat([scores[:self.ignore_index], scores[self.ignore_index+1:]])
...
class PanopticMetric(Metric):
    def __init__(
        ...
        self.add_state('iou', default=jittor.zeros(n_classes), dist_reduce_fx='sum')
        self.add_state('true_positive', default=jittor.zeros(n_classes), dist_reduce_fx='sum')
        self.add_state('false_positive', default=jittor.zeros(n_classes), dist_reduce_fx='sum')
        self.add_state('false_negative', default=jittor.zeros(n_classes), dist_reduce_fx='sum')
    ...
    def compute(self):
        denominator = jittor.maximum(
            (self.true_positive + self.false_positive / 2 + self.false_negative / 2),
            jittor.ones_like(self.true_positive)
        )
        pq = self.iou / denominator
        sq = self.iou / jittor.maximum(self.true_positive, jittor.ones_like(self.true_positive))
    ...
    def panoptic_metrics(self, pred_segmentation, pred_instance, gt_segmentation, gt_instance, unique_id_mapping):
        ...
        result = {key: jittor.zeros(n_classes, dtype=jittor.float32) for key in self.keys}

        assert pred_segmentation.dim() == 2
        assert pred_segmentation.shape == pred_instance.shape == gt_segmentation.shape == gt_instance.shape

        n_instances = int(jittor.concat([pred_instance, gt_instance]).max().item())
        ...
        # TODO: metric 最终需要参考是否涉及梯度回传
        x = np.int64(x.numpy())
        bincount_2d = np.bincount(x, minlength=n_things_and_void ** 2)
        bincount_2d = jittor.array(bincount_2d)
        ...
        iou = jittor.where(union > 0, (conf.float() + 1e-9) / (union.float() + 1e-9), jittor.zeros_like(union).float())
        ...
        for target_id, pred_id in mapping:
            ...
            result['iou'][cls_id] += iou[int(target_id)][int(pred_id)]
            unique_id_mapping[target_id.item()] = pred_id.item()

        for target_id in range(n_classes, n_all_things):
            # If this is a true positive do nothing.
            if tp_mask[target_id, n_classes:].to(jittor.bool).any():
                continue
        ...
        for pred_id in range(n_classes, n_all_things):
            # If this is a true positive do nothing.
            if tp_mask[n_classes:, pred_id].to(jittor.bool).any():
                continue
    ...
    def combine_mask(self, segmentation: jittor.Var, instance: jittor.Var, n_classes: int, n_all_things: int):
        ...
        instance_id_to_class_tuples = jittor.concat(
            (
        ...
        instance_id_to_class[jittor.arange(n_classes)] = jittor.arange(
            n_classes
        )

        segmentation[instance_mask] = instance[instance_mask]
        segmentation += 1  # Shift all legit classes by 1.
        segmentation[jittor.array(~(segmentation_mask.data))] = 0  # Shift void class to zero.
