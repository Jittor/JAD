...
def calculate_birds_eye_view_parameters(x_bounds, y_bounds, z_bounds):
    ...
    """
    bev_resolution = jittor.array(
        [row[2] for row in [x_bounds, y_bounds, z_bounds]])
    bev_start_position = jittor.array(
        [row[0] + row[2] / 2.0 for row in [x_bounds, y_bounds, z_bounds]])
    bev_dimension = jittor.array([(row[1] - row[0]) / row[2]
                                 for row in [x_bounds, y_bounds, z_bounds]], dtype=jittor.int32)

    return bev_resolution, bev_start_position, bev_dimension


def gen_dx_bx(xbound, ybound, zbound):
    dx = jittor.array([row[2] for row in [xbound, ybound, zbound]])
    bx = jittor.array([row[0] + row[2]/2.0 for row in [xbound, ybound, zbound]])
    nx = jittor.array([(row[1] - row[0]) / row[2] for row in [xbound, ybound, zbound]], dtype=jittor.int64)
...
def update_instance_ids(instance_seg, old_ids, new_ids):
    ...
    Parameters
    ----------
        instance_seg: jittor.Var arbitrary shape
    ...
    Returns
        new_instance_seg: jittor.Var same shape as instance_seg with new ids
    """
    indices = jittor.arange(old_ids.max() + 1)
...
def make_instance_seg_consecutive(instance_seg):
    ...
    unique_ids = jittor.unique(instance_seg)  # include background
    new_ids = jittor.arange(len(unique_ids))
...
def predict_instance_segmentation_and_trajectories(
    ...
    argmax_ins = ins_sigmoid.argmax(dim=1)[0]  # long, [b, t, h, w], ins_id starts from 0
