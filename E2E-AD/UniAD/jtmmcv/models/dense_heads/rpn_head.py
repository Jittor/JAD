...
from jtmmcv.ops.nms import batched_nms
...
class RPNHead(AnchorHead):
    ...
    def _init_layers(self):
        ...
        self.rpn_conv = jnn.Conv2d(
            self.in_channels, self.feat_channels, 3, padding=1)
        self.rpn_cls = jnn.Conv2d(self.feat_channels,
                                 self.num_anchors * self.cls_out_channels, 1)
        self.rpn_reg = jnn.Conv2d(self.feat_channels, self.num_anchors * 4, 1)
    ...
    def _get_bboxes_single(self,
        ...
        for idx in range(len(cls_scores)):
            ...
            level_ids.append(
                scores.new_full((scores.size(0), ), idx))

        scores = jittor.concat(mlvl_scores)
        anchors = jittor.concat(mlvl_valid_anchors)
        rpn_bbox_pred = jittor.concat(mlvl_bbox_preds)
        proposals = self.bbox_coder.decode(
            anchors, rpn_bbox_pred, max_shape=img_shape)
        ids = jittor.concat(level_ids)
    ...
    def onnx_export(self, x, img_metas):
        ...
        nms_pre_tensor = jittor.array(
            cfg.nms_pre, dtype=jittor.int32)
        for idx in range(len(cls_scores)):
            rpn_cls_score = cls_scores[idx]
            rpn_bbox_pred = bbox_preds[idx]
            assert rpn_cls_score.size()[-2:] == rpn_bbox_pred.size()[-2:]
            ...
            rpn_bbox_pred = rpn_bbox_pred.permute(0, 2, 3, 1).reshape(
                batch_size, -1, 4)
            anchors = mlvl_anchors[idx]
            anchors = anchors.expand(rpn_bbox_pred.size())
        ...
        batch_mlvl_anchors = jittor.concat(mlvl_valid_anchors, dim=1)
        batch_mlvl_rpn_bbox_pred = jittor.concat(mlvl_bbox_preds, dim=1)
        ...
        from jtmmcv.core.export import add_dummy_nms_for_onnx
