...
from jtmmcv import is_tuple_of
from jtmmcv.models.bricks import ConvModule
from jittor import nn as jnn

from jtmmcv.models.builder import build_loss
...
class VoteModule(Module):
    ...
    def __init__(self,
        ...
        self.vote_conv = jnn.Sequential(*vote_conv_list)
        ...
        self.conv_out = jnn.Conv1d(prev_channels, out_channel, 1)

    def execute(self, seed_points, seed_feats):
        """forward.

        Args:
            seed_points (jittor.Var): Coordinate of the seed
                points in shape (B, N, 3).
            seed_feats (jittor.Var): Features of the seed points in shape
                (B, C, N).
        ...
        if self.vote_xyz_range is not None:
            ...
            for axis in range(len(self.vote_xyz_range)):
                limited_offset_list.append(offset[..., axis].clamp(
                    min_v=-self.vote_xyz_range[axis],
                    max_v=self.vote_xyz_range[axis]))
            limited_offset = jittor.stack(limited_offset_list, -1)
        ...
        if self.with_res_feat:
            ...
            if self.norm_feats:
                features_norm = jittor.norm(vote_feats, p=2, dim=1)
    ...
    def get_loss(self, seed_points, vote_points, seed_indices,
        ...
        Args:
            seed_points (jittor.Var): Coordinate of the seed points.
            vote_points (jittor.Var): Coordinate of the vote points.
            seed_indices (jittor.Var): Indices of seed points in raw points.
            vote_targets_mask (jittor.Var): Mask of valid vote targets.
            vote_targets (jittor.Var): Targets of votes.

        Returns:
            jittor.Var: Weighted vote loss.
        """
        batch_size, num_seed = seed_points.shape[:2]

        seed_gt_votes_mask = jittor.gather(vote_targets_mask, 1,
                                          seed_indices).float()

        seed_indices_expand = seed_indices.unsqueeze(-1).repeat(
            1, 1, 3 * self.gt_per_seed)
        seed_gt_votes = jittor.gather(vote_targets, 1, seed_indices_expand)
        seed_gt_votes += seed_points.repeat(1, 1, self.gt_per_seed)

        weight = seed_gt_votes_mask / (jittor.sum(seed_gt_votes_mask) + 1e-6)
        ...
        vote_loss = jittor.sum(jittor.min(distance, dim=1)[0])
