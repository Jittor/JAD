...
import jittor
from jtmmcv.ops.roi_align import roi_align
from jtmmcv.image import rescale_size, imrescale, imresize, imflip, impad, imtranslate, imshear, imrotate
...
class BaseInstanceMasks(metaclass=ABCMeta):
    ...
    def to_tensor(self, dtype, device):
        ...
        Args:
            ...
            device (jittor.device): Device of converted masks.
...
class BitmapMasks(BaseInstanceMasks):
    ...
    Example:
        >>> from jtmmcv.core.mask.structures import *  # NOQA
        ...
        >>> out_shape = (14, 14)
        >>> inds = jittor.randint(0, len(self), size=(num_boxes,))
        >>> device = 'cpu'
        >>> interpolation = 'bilinear'
    ...
    def crop_and_resize(self,
        ...
        if isinstance(bboxes, np.ndarray):
            bboxes = jittor.array(bboxes).to(device)
        if isinstance(inds, np.ndarray):
            inds = jittor.array(inds).to(device)

        num_bbox = bboxes.shape[0]
        fake_inds = jittor.arange(
            num_bbox).to(dtype=bboxes.dtype)[:, None]
        rois = jittor.concat([fake_inds, bboxes], dim=1)  # Nx5
        rois = rois.to(device)
        if num_bbox > 0:
            gt_masks_th = jittor.array(self.masks).to(device).index_select(
                0, inds).to(dtype=rois.dtype)
            targets = roi_align(gt_masks_th[:, None, :, :], rois, out_shape,
                                1.0, 0, 'avg', True).squeeze(1)
            if binarize:
                resized_masks = (targets >= 0.5).numpy()
            else:
                resized_masks = targets.numpy()
        else:
            resized_masks = []
        return BitmapMasks(resized_masks, *out_shape)
    ...
    def translate(self,
        ...
        Example:
            >>> from jtmmcv.core.mask.structures import BitmapMasks
    ...
    def to_tensor(self, dtype, device):
        ...
        return jittor.Var(self.masks, dtype=dtype)
    ...
    def random(cls,
        ...
        """
        from jtmmcv.utils.general import ensure_rng
        rng = ensure_rng(rng)
...
class PolygonMasks(BaseInstanceMasks):
    ...
    def to_tensor(self, dtype, device):
        """See :func:`BaseInstanceMasks.to_tensor`."""
        if len(self.masks) == 0:
            return jittor.empty((0, self.height, self.width),
                               dtype=dtype)
        ndarray_masks = self.to_ndarray()
        return jittor.Var(ndarray_masks, dtype=dtype)
    ...
    def random(cls,
        ...
        Example:
            >>> from jtmmcv.core.mask.structures import PolygonMasks
            >>> self = PolygonMasks.random()
            >>> print('self = {}'.format(self))
        """
        from jtmmcv.utils.util_random import ensure_rng
        rng = ensure_rng(rng)
        ...
        def _order_vertices(verts):
            ...
            """
            mlat = verts.transpose()[0].sum() / len(verts)
            mlng = verts.transpose()[1].sum() / len(verts)

            tau = np.pi * 2
            angle = (np.arctan2(mlat - verts.transpose()[0], verts.transpose()[1] - mlng) +
                     tau) % tau
            sortx = angle.argsort()
