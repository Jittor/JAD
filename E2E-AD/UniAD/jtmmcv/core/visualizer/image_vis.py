...
def project_pts_on_img(points,
    ...
    pts_2d = pts_4d @ lidar2img_rt.transpose()
...
def draw_lidar_bbox3d_on_img(bboxes3d,
    ...
    if isinstance(lidar2img_rt, jittor.Var):
        lidar2img_rt = lidar2img_rt.numpy()
    pts_2d = pts_4d @ lidar2img_rt.transpose()
...
def draw_depth_bbox3d_on_img(bboxes3d,
    ...
    Args:
        ...
        thickness (int, optional): The thickness of bboxes. Default: 1.
    """
    from jtmmcv.core.bbox import points_cam2img
    from jtmmcv.models import apply_3d_transformation
    ...
    uv_origin = points_cam2img(xyz_depth,
                               jittor.array(img_metas['depth2img']).to(dtype=xyz_depth.dtype))
...
def draw_camera_bbox3d_on_img(bboxes3d,
    ...
    num_bbox = corners_3d.shape[0]
    points_3d = corners_3d.reshape(-1, 3)
    if not isinstance(cam2img, jittor.Var):
        cam2img = jittor.array(np.array(cam2img))
    cam2img = cam2img.reshape(3, 3).float()
