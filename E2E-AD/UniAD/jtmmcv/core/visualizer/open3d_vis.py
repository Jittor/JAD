...
def _draw_points(points,
    ...
    Args:
        points (numpy.array | jittor.Var, shape=[N, 3+C]):
            points to visualize.
...
def _draw_bboxes(bbox3d,
    ...
    Args:
        bbox3d (numpy.array | jittor.Var, shape=[M, 7]):
        ...
        mode (str):  indicate type of the input points, avaliable mode
            ['xyz', 'xyzrgb']. Default: 'xyz'.
    """
    if isinstance(bbox3d, jittor.Var):
        bbox3d = bbox3d.numpy()
    bbox3d = bbox3d.copy()
...
def _draw_bboxes_ind(bbox3d,
    ...
    Args:
        bbox3d (numpy.array | jittor.Var, shape=[M, 7]):
            3d bbox (x, y, z, dx, dy, dz, yaw) to visualize.
        vis (:obj:`open3d.visualization.Visualizer`): open3d visualizer.
        indices (numpy.array | jittor.Var, shape=[N, M]):
    ...
    if isinstance(bbox3d, jittor.Var):
        bbox3d = bbox3d.numpy()
    if isinstance(indices, jittor.Var):
