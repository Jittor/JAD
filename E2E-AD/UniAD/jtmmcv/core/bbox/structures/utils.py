...
def limit_period(val, offset=0.5, period=np.pi):
    ...
    Args:
        val (jittor.Var): The value to be converted.
        offset (float, optional): Offset to set the value range. \
            Defaults to 0.5.
        period ([type], optional): Period of the value. Defaults to np.pi.

    Returns:
        jittor.Var: Value in the range of \
            [-offset * period, (1-offset) * period]
    """
    return val - jittor.floor(val / period + offset) * period
...
def rotation_3d_in_axis(points, angles, axis=0):
    ...
    Args:
        points (jittor.Var): Points of shape (N, M, 3).
        angles (jittor.Var): Vector of angles in shape (N,)
    ...
    Returns:
        jittor.Var: Rotated points in shape (N, M, 3)
    """
    rot_sin = jittor.sin(angles)
    rot_cos = jittor.cos(angles)
    ones = jittor.ones_like(rot_cos)
    zeros = jittor.zeros_like(rot_cos)
    if axis == 1:
        rot_mat_T = jittor.stack([
            jittor.stack([rot_cos, zeros, -rot_sin]),
            jittor.stack([zeros, ones, zeros]),
            jittor.stack([rot_sin, zeros, rot_cos])
        ])
    elif axis == 2 or axis == -1:
        rot_mat_T = jittor.stack([
            jittor.stack([rot_cos, -rot_sin, zeros]),
            jittor.stack([rot_sin, rot_cos, zeros]),
            jittor.stack([zeros, zeros, ones])
        ])
    elif axis == 0:
        rot_mat_T = jittor.stack([
            jittor.stack([zeros, rot_cos, -rot_sin]),
            jittor.stack([zeros, rot_sin, rot_cos]),
            jittor.stack([ones, zeros, zeros])
        ])
    else:
        raise ValueError(f'axis should in range [0, 1, 2], got {axis}')

    return jittor.linalg.einsum('aij,jka->aik', points, rot_mat_T)
...
def xywhr2xyxyr(boxes_xywhr):
    ...
    Args:
        boxes_xywhr (jittor.Var): Rotated boxes in XYWHR format.

    Returns:
        jittor.Var: Converted boxes in XYXYR format.
    """
    boxes = jittor.zeros_like(boxes_xywhr)
...
def points_cam2img(points_3d, proj_mat, with_depth=False):
    ...
    Args:
        points_3d (jittor.Var): Points in shape (N, 3).
        proj_mat (jittor.Var): Transformation matrix between coordinates.
        with_depth (bool, optional): Whether to keep depth in the output.
            Defaults to False.

    Returns:
        jittor.Var: Points in image coordinates with shape [N, 2].
    """
    ...
    if d1 == 3:
        proj_mat_expanded = jittor.init.eye(
            4, dtype=proj_mat.dtype)
        proj_mat_expanded[:d1, :d2] = proj_mat
        proj_mat = proj_mat_expanded

    # previous implementation use new_zeros, new_one yeilds better results
    points_4 = jittor.concat(
        [points_3d, points_3d.new_ones(*points_shape)], dim=-1)
    point_2d = jittor.matmul(points_4, proj_mat.t())
    point_2d_res = point_2d[..., :2] / point_2d[..., 2:3]

    if with_depth:
        return jittor.concat([point_2d_res, point_2d[..., 2:3]], dim=-1)
    return point_2d_res
...
def mono_cam_box2vis(cam_box):
    ...
    yaw = -yaw - np.pi / 2
    cam_box = jittor.concat([loc, dim, yaw[:, None], feats], dim=1)
...
def get_proj_mat_by_coord_type(img_meta, coord_type):
    ...
    Returns:
        jittor.Var: transformation matrix.
    """
