...
from jtmmcv.models.dense_heads.occ_head_plugin import calculate_birds_eye_view_parameters
...
class GenerateOccFlowLabels(object):
    ...
    def reframe_boxes(self, boxes, t_init, t_curr):
        ...
        boxes.rotate(l2e_r_mat_curr.transpose())
        boxes.translate(l2e_t_curr)

        # to bbox under world frame
        boxes.rotate(e2g_r_mat_curr.transpose())
    ...
    def __call__(self, results):
        ...
        segmentations = jittor.array(
            np.stack(segmentations, axis=0)).long()
        instances = jittor.array(np.stack(instances, axis=0)).long()
    ...
    def center_offset_flow(self, instance_img, all_gt_inds, ignore_index=255, sigma=3.0):
        ...
        # heatmap
        center_label = jittor.zeros((seq_len, 1, h, w))
        # offset from parts to centers
        offset_label = ignore_index * jittor.ones((seq_len, 2, h, w))
        # future flow
        future_displacement_label = ignore_index * jittor.ones(seq_len, 2, h, w)

        # backward flow
        backward_flow = ignore_index * jittor.ones(seq_len, 2, h, w)

        # x is vertical displacement, y is horizontal displacement
        x, y = jittor.meshgrid(jittor.arange(h, dtype=jittor.float32),
                            jittor.arange(w, dtype=jittor.float32))
        ...
        for instance_id in gt_inds_unique:
            ...
            for t in range(seq_len):
                ...
                off_x = xc - x
                off_y = yc - y
                g = jittor.exp(-(off_x ** 2 + off_y ** 2) / sigma ** 2)
                center_label[t, 0] = jittor.maximum(center_label[t, 0], g)
