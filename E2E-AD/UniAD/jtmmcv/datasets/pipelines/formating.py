...
from jtmmcv.core.bbox.structures.base_box3d import BaseInstance3DBoxes
from jtmmcv.core.points import BasePoints
from jtmmcv.utils import is_str
...
def to_tensor(data):
    ...
    Supported types are: :class:`numpy.ndarray`, :class:`jittor.Var`,
    :class:`Sequence`, :class:`int` and :class:`float`.

    Args:
        data (jittor.Var | numpy.ndarray | Sequence | int | float): Data to
            be converted.
    """

    if isinstance(data, jittor.Var):
        return data
    elif isinstance(data, np.ndarray):
        return jittor.array(data)
...
class ToTensor:
    """Convert some results to :obj:`jittor.Var` by given keys.
    ...
    def __call__(self, results):
        """Call function to convert data in results to :obj:`jittor.Var`.
...
@PIPELINES.register_module()
class ImageToTensor:
    """Convert image to :obj:`jittor.Var` by given keys.
    ...
    def __call__(self, results):
        """Call function to convert image in results to :obj:`jittor.Var` and
        ...
        Returns:
            dict: The result dict contains the image converted
                to :obj:`jittor.Var` and transposed to (C, H, W) order.
        """
        for key in self.keys:
            img = results[key]
            if len(img.shape) < 3:
