...
def prediction_metrics(gt_box_match, pred_box):
    ...
    pred_traj = jittor.Var(pred_traj[None])
    gt_traj = jittor.Var(gt_traj[None])
    valid_mask = jittor.Var(valid_mask[None])
...
def accumulate(gt_boxes: EvalBoxes,
    ...
    for key in match_data.keys():
        if key == "conf":
            ...
            for i in range(len(match_data[key])):
                if isinstance(match_data[key][i], int) or isinstance(match_data[key][i],float):
                    match_data[key][i]=np.array(match_data[key][i]).reshape(1,)
