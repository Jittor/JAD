...
from jtmmcv.datasets.nuscenes_e2e_dataset import obtain_map_info
from jtmmcv.datasets.eval_utils.map_api import NuScenesMap
from jtmmcv.fileio.io import load
    ...
    def _parse_predictions_multitask_pkl(self, predroot):
        ...
        for k in range(len(outputs)):
            ...
            if self.show_sdc_traj:
                outputs[k]['boxes_3d'].tensor = jittor.concat(
                    [outputs[k]['boxes_3d'].tensor, outputs[k]['sdc_boxes_3d'].tensor], dim=0)
                outputs[k]['scores_3d'] = jittor.concat(
                    [outputs[k]['scores_3d'], outputs[k]['sdc_scores_3d']], dim=0)
                outputs[k]['labels_3d'] = jittor.concat([outputs[k]['labels_3d'], jittor.zeros(
            ...
            track_scores = scores.detach().numpy()
            track_labels = labels.detach().numpy()
            track_boxes = bboxes.tensor.detach().numpy()

            track_centers = bboxes.gravity_center.detach().numpy()
            track_dims = bboxes.dims.detach().numpy()
            track_yaw = bboxes.yaw.detach().numpy()

            if 'track_ids' in outputs[k]:
                track_ids = outputs[k]['track_ids'].detach().numpy()
            else:
                track_ids = None

            # speed
            track_velocity = bboxes.tensor.detach().numpy()[:, -2:]
            ...
            if self.with_map:
                map_thres = 0.7
                score_list = outputs[k]['pts_bbox']['score_list'].numpy().transpose([
                    1, 2, 0])
                predicted_map_seg = outputs[k]['pts_bbox']['lane_score'].numpy().transpose([
                    1, 2, 0])  # H, W, C
            ...
            if self.with_planning:
                ...
                if self.show_command:
                    command = outputs[k]['command'][0].detach().numpy()
                else:
                    command = None
                planning_agent = AgentPredictionData(
                    ...
                    track_yaw[0],
                    track_velocity[0],
                    outputs[k]['planning_traj'][0].detach().numpy(),
                    1,
                    pred_track_id=-1,
                    pred_occ_map=None,
                    past_pred_traj=None,
