...
from jtmmcv.core.bbox.box_np_ops import points_cam2img
from jtmmcv.datasets import NuScenesDataset
from jtmmcv.fileio.io import load, dump
from jtmmcv.utils import is_filepath, track_iter_progress, check_file_exist
...
def create_nuscenes_infos(root_path,
    ...
    """
    from nuscenes.nuscenes import NuScenes
    if can_bus_root_path is not None:
...
def _fill_trainval_infos(nusc,
    ...
    for sample in track_iter_progress(nusc.sample):
        ...
        ##
            ...
            print("Didn't load can_bus information!")
            info = {
            'lidar_path': lidar_path,
            'token': sample['token'],
            'prev': sample['prev'],
        ...
        if not test:
            ...
            for i in range(len(boxes)):
                ...
                velo = velo @ np.linalg.inv(e2g_r_mat).transpose() @ np.linalg.inv(
                    l2e_r_mat).transpose()
                velocity[i] = velo[:2]
...
def obtain_sensor2top(nusc,
    ...
    R = (l2e_r_s_mat.transpose() @ e2g_r_s_mat.transpose()) @ (
        np.linalg.inv(e2g_r_mat).transpose() @ np.linalg.inv(l2e_r_mat).transpose())
    T = (l2e_t_s @ e2g_r_s_mat.transpose() + e2g_t_s) @ (
        np.linalg.inv(e2g_r_mat).transpose() @ np.linalg.inv(l2e_r_mat).transpose())
    T -= e2g_t @ (np.linalg.inv(e2g_r_mat).transpose() @ np.linalg.inv(l2e_r_mat).transpose()
                  ) + l2e_t @ np.linalg.inv(l2e_r_mat).transpose()
    sweep['sensor2lidar_rotation'] = R.transpose()  # points @ R.transpose() + T
    ...
    for ann_rec in ann_recs:
        ...
        if mono3d and (repro_rec is not None):
            ...
            cam_velo3d = global_velo3d @ np.linalg.inv(
                e2g_r_mat).transpose() @ np.linalg.inv(c2e_r_mat).transpose()
