...
from mtr_jittor.datasets import build_dataloader
from mtr_jittor.config import cfg, cfg_from_list, cfg_from_yaml_file, log_config_to_file
from mtr_jittor.utils import common_utils
from mtr_jittor.models import model as model_utils
...
def parse_config():
    ...
    parser.add_argument(
        "--launcher", choices=["none", "pytorch", "slurm", "jittor"], default="none"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--not_eval_with_train", action="store_true", default=False, help=""
    )
    ...
    parser.add_argument(
        "--add_worker_init_fn", action="store_true", default=False, help=""
    )
...
def build_scheduler(
    ...
    if opt_cfg.get("SCHEDULER", None) == "cosine":
        scheduler = lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=2 * len(dataloader),
            eta_min=max(1e-2 * opt_cfg.LR, 1e-6),
            last_epoch=-1,
        )
    elif opt_cfg.get("SCHEDULER", None) == "lambdaLR":
        scheduler = optim.LambdaLR(optimizer, lr_lbmd, last_epoch=last_epoch)
