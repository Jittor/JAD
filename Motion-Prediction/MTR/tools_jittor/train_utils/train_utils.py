...
def train_one_epoch(
    ...
    optimizer, optimizer_2 = (
        optimizer if isinstance(optimizer, list) else (optimizer, None)
    )
    if rank == 0:
        pbar = tqdm.tqdm(
    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        optimizer.clip_grad_norm(optim_cfg.GRAD_NORM_CLIP)
        if optimizer_2 is not None:
            optimizer_2.clip_grad_norm(optim_cfg.GRAD_NORM_CLIP)
        ...
        if rank == 0:
            if (
                ...
                logger.info(
                    f"epoch: {cur_epoch}/{total_epochs}, acc_iter={accumulated_iter}, cur_iter={cur_it}/{total_it_each_epoch}, batch_size={batch_size}, iter_cost={second_each_iter:.2f}s, "
...
def train_model(
    ...
    with tqdm.trange(
        ...
        for cur_epoch in tbar:
            ...
            if (
                trained_epoch % ckpt_save_interval == 0
                or trained_epoch in [1, 2, 4]
                or trained_epoch > total_epochs - 10
            ) and rank == 0:
                ...
                if cfg.LOCAL_RANK == 0:
                    ...
                    if "mAP" in tb_dict:
                        ...
                        try:
                            ...
                            best_performance = (
                                best_src_data[-1].strip().split(" ")[-1]
                            )  # best_epoch_xx MissRate 0.xx
                        ...
                        if best_performance == -1 or tb_dict["mAP"] > float(
                            best_performance
                        ):
                            ckpt_name = ckpt_save_dir / "best_model"
                            save_checkpoint(
                                checkpoint_state(
                                    model, epoch=cur_epoch, it=accumulated_iter
                                ),
                                filename=ckpt_name,
                            )
                            ...
                            with open(best_record_file, "a") as f:
                                print(
                                    f'best_epoch_{trained_epoch} mAP {tb_dict["mAP"]}',
                                    file=f,
                                )
                        else:
                            with open(best_record_file, "a") as f:
