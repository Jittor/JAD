...
def nll_loss_gmm_direct(
    ...
    if pre_nearest_mode_idxs is not None:
        ...
        nearest_mode_idxs = distance.argmin(dim=-1)[0]
    nearest_mode_bs_idxs = jittor.arange(batch_size).type_as(
        nearest_mode_idxs
    )  # (batch_size, 2)
    ...
    if use_square_gmm:
        log_std1 = log_std2 = jittor.clamp(
            nearest_trajs[:, :, 2], min_v=log_std_range[0], max_v=log_std_range[1]
        )
        std1 = std2 = jittor.exp(log_std1)  # (0.2m to 150m)
        rho = jittor.zeros_like(log_std1)
    else:
        log_std1 = jittor.clamp(
            nearest_trajs[:, :, 2], min_v=log_std_range[0], max_v=log_std_range[1]
        )
        log_std2 = jittor.clamp(
            nearest_trajs[:, :, 3], min_v=log_std_range[0], max_v=log_std_range[1]
        )
        std1 = jittor.exp(log_std1)  # (0.2m to 150m)
        std2 = jittor.exp(log_std2)  # (0.2m to 150m)
        rho = jittor.clamp(nearest_trajs[:, :, 4], min_v=-rho_limit, max_v=rho_limit)
    ...
    reg_gmm_log_coefficient = (
        log_std1 + log_std2 + 0.5 * jittor.log(1 - rho**2)
