...
def batch_nms(pred_trajs, pred_scores, dist_thresh, num_ret_modes=6):
    ...
    sorted_idxs = pred_scores.argsort(dim=-1, descending=True)[0]
    bs_idxs_full = (
        jittor.arange(batch_size).type_as(sorted_idxs)[:, None].repeat(1, num_modes)
    )
    ...
    point_val_selected = jittor.zeros_like(point_val)  # (batch_size, N)

    ret_idxs = jittor.zeros((batch_size, num_ret_modes), dtype=sorted_idxs.dtype).long()
    ret_trajs = jittor.zeros(
        (batch_size, num_ret_modes, num_timestamps, num_feat_dim), dtype=sorted_pred_trajs.dtype
    )
    ret_scores = jittor.zeros((batch_size, num_ret_modes), dtype=sorted_pred_scores.dtype)
    bs_idxs = jittor.arange(batch_size).type_as(ret_idxs)

    for k in range(num_ret_modes):
        cur_idx = point_val.argmax(dim=-1)[0]  # (batch_size)
        ret_idxs[:, k] = cur_idx

        new_cover_mask = point_cover_mask[bs_idxs, cur_idx]  # (batch_size, N)
        # point_val = point_val * (~new_cover_mask).float()  # (batch_size, N)
        point_val = point_val * (new_cover_mask.bitwise_xor(1)).float()
    ...
    bs_idxs = (
        jittor.arange(batch_size).type_as(sorted_idxs)[:, None].repeat(1, num_ret_modes)
    )
...
def get_ade_of_waymo(
    pred_trajs, gt_trajs, gt_valid_mask, calculate_steps=(5, 9, 15)
) -> float:
    ...
    for cur_step in calculate_steps:
        dist_error = (
            pred_trajs[:, :, : cur_step + 1, :] - gt_trajs[:, None, : cur_step + 1, :]
        ).norm(
            dim=-1
        )  # (batch_size, num_modes, pred_len)
        dist_error = (dist_error * gt_valid_mask[:, None, : cur_step + 1].float()).sum(
            dim=-1
        ) / jittor.clamp(
            gt_valid_mask[:, : cur_step + 1].sum(dim=-1)[:, None], min_v=1.0
