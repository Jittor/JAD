...
from mtr_jittor.ops import attention
...
class MultiheadAttentionLocal(nn.Module):
    ...
    def __init__(
        ...
        assert version in [
            "v1",
            "v2",
        ], "only attention_utils_v1 and attention_utils_v2 are available."
    ...
    def execute(
        ...
        **kwargs
        ...
        if attn_mask is not None:
            ...
            used_attn_mask = jittor.logical_or(used_attn_mask, attn_mask)
        ...
        attn_output_weights = attention.__all__[
            self.attention_version
        ...
        used_attn_mask = (
            used_attn_mask.unsqueeze(-1).repeat(1, 1, self.num_heads).contiguous()
        )
        attn_output_weights.masked_fill_(used_attn_mask, float("-inf"))
        attn_output_weights = nn.softmax(attn_output_weights, dim=1)
        attn_output_weights = nn.dropout(
            attn_output_weights, p=self.dropout, is_train=self.training
        )
