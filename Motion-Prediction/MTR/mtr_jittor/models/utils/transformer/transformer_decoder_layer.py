...
class TransformerDecoderLayer(nn.Module):
    ...
    def with_pos_embed(self, tensor, pos: Optional[Var]):
    ...
    def execute(
        ...
        pos: Optional[Var] = None,
        query_pos: Optional[Var] = None,
        ...
        if not self.rm_self_attn_decoder:
            ...
            tgt2 = self.self_attn(
                q, k, value=v, attn_mask=tgt_mask, key_padding_mask=None
            )[0]
        ...
        if self.use_local_attn:
            ...
            query_batch_cnt = jittor.zeros_like(key_batch_cnt)
        ...
        if self.use_local_attn and memory_valid_mask is not None:
            ...
            k_content = jittor.new_zeros(memory,(memory.shape[0], k_content_valid.shape[-1]))
            k_content[memory_valid_mask] = k_content_valid

            v_valid = self.ca_v_proj(valid_memory)
            v = jittor.new_zeros(memory,(memory.shape[0], v_valid.shape[-1]))
            v[memory_valid_mask] = v_valid

            valid_pos = pos[memory_valid_mask]
            k_pos_valid = self.ca_kpos_proj(valid_pos)
            k_pos = jittor.new_zeros(pos,(pos.shape[0], k_pos_valid.shape[-1]))
        ...
        if self.use_local_attn:
            ...
            q = jittor.concat([q, query_sine_embed], dim=3).view(
                num_queries, bs, n_model * 2
            )

            k = k.view(hw, bs, self.nhead, n_model // self.nhead)
            k_pos = k_pos.view(hw, bs, self.nhead, n_model // self.nhead)
            k = jittor.concat([k, k_pos], dim=3).view(hw, bs, n_model * 2)
