...
class NuscenesDD3DTargetPreparer(DD3DTargetPreparer):
    ...
    def __call__(self, locations, gt_instances, feature_shapes):
        ...
        for l, loc_per_level in enumerate(locations):
            loc_to_size_range_per_level = loc_per_level.new(self.sizes_of_interest[l])
...
class NuscenesLoss(nn.Module):
    ...
    def forward(self, attr_logits, speeds, fcos2d_info, targets):
        ...
        if pos_inds.numel() == 0:
            attr_weights = attr_logits.new(0.0) #torch.tensor(0.0).cuda()
        else:
        ...
        if pos_inds.numel() == 0:
            speed_weights = speeds.new(0.0) #torch.tensor(0.0).cuda()
        else:
        ...
        for lvl, (attr_logits_lvl, speed_lvl, info_lvl, instances_lvl) in \
            ...
            for i in range(N):
                ...
                if len(attr_logits_per_im) == 0:
                    instances_lvl[i].pred_attributes = instances_lvl[i].pred_classes.new([])
                    instances_lvl[i].pred_speeds = instances_lvl[i].scores.new([])
                else:
