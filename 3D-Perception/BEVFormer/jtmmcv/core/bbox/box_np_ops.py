...
def camera_to_lidar(points, r_rect, velo2cam):
    ...
    lidar_points = points @ np.linalg.inv((r_rect @ velo2cam).transpose())
...
def depth_to_lidar_points(depth, trunc_pixel, P2, r_rect, velo2cam):
    ...
    points = points @ np.linalg.inv(P2.transpose())
...
def points_cam2img(points_3d, proj_mat, with_depth=False):
    ...
    point_2d = points_4 @ proj_mat.transpose()
...
def create_anchors_3d_range(feature_size,
    ...
    Args:
        ...
        sizes (list[list] | np.ndarray | jittor.Var, optional):
            Anchor size with shape [N, 3], in order of x, y, z.
            Defaults to ((1.6, 3.9, 1.56), ).
        rotations (list[float] | np.ndarray | jittor.Var, optional):
...
def remove_outside_points(points, rect, Trv2c, P2, image_shape):
    ...
    frustum -= T
    frustum = np.linalg.inv(R) @ frustum.transpose()
    frustum = camera_to_lidar(frustum.transpose(), rect, Trv2c)
