...
class BboxOverlapsNearest3D(object):
    ...
    def __call__(self, bboxes1, bboxes2, mode='iou', is_aligned=False):
        ...
        Args:
            bboxes1 (jittor.Var): shape (N, 7+N) [x, y, z, h, w, l, ry, v].
            bboxes2 (jittor.Var): shape (M, 7+N) [x, y, z, h, w, l, ry, v].
            mode (str): "iou" (intersection over union) or iof
                (intersection over foreground).
            is_aligned (bool): Whether the calculation is aligned.

        Return:
            jittor.Var: If ``is_aligned`` is ``True``, return ious between \
...
class BboxOverlaps3D(object):
    ...
    def __call__(self, bboxes1, bboxes2, mode='iou'):
        ...
        Args:
            bboxes1 (jittor.Var): shape (N, 7+C) [x, y, z, h, w, l, ry].
            bboxes2 (jittor.Var): shape (M, 7+C) [x, y, z, h, w, l, ry].
            mode (str): "iou" (intersection over union) or
                iof (intersection over foreground).

        Return:
            jittor.Var: Bbox overlaps results of bboxes1 and bboxes2 \
...
def bbox_overlaps_nearest_3d(bboxes1,
    ...
    Args:
        bboxes1 (jittor.Var): shape (N, 7+C) [x, y, z, h, w, l, ry, v].
        bboxes2 (jittor.Var): shape (M, 7+C) [x, y, z, h, w, l, ry, v].
        mode (str): "iou" (intersection over union) or iof
            (intersection over foreground).
        is_aligned (bool): Whether the calculation is aligned
    ...
    # box conversion and iou calculation in jittor version on CUDA
...
def axis_aligned_bbox_overlaps_3d(bboxes1,
    ...
    Example:
        >>> empty = jittor.empty(0, 6)
        >>> nonempty = jittor.Var([[0, 0, 0, 10, 9, 10]])
    ...
    if is_aligned:
        lt = jittor.max(bboxes1[..., :3], bboxes2[..., :3])  # [B, rows, 3]
        rb = jittor.min(bboxes1[..., 3:], bboxes2[..., 3:])  # [B, rows, 3]
        ...
        if mode in ['iou', 'giou']:
            ...
            union = area1
        if mode == 'giou':
            enclosed_lt = jittor.min(bboxes1[..., :3], bboxes2[..., :3])
            enclosed_rb = jittor.max(bboxes1[..., 3:], bboxes2[..., 3:])
    else:
        lt = jittor.max(bboxes1[..., :, None, :3],
                       bboxes2[..., None, :, :3])  # [B, rows, cols, 3]
        rb = jittor.min(bboxes1[..., :, None, 3:],
                       bboxes2[..., None, :, 3:])  # [B, rows, cols, 3]

        wh = (rb - lt).clamp(min_v=0)  # [B, rows, cols, 3]
        overlap = wh[..., 0] * wh[..., 1] * wh[..., 2]

        if mode in ['iou', 'giou']:
            union = area1[..., None] + area2[..., None, :] - overlap
        if mode == 'giou':
            enclosed_lt = jittor.min(bboxes1[..., :, None, :3],
                                    bboxes2[..., None, :, :3])
            enclosed_rb = jittor.max(bboxes1[..., :, None, 3:],
                                    bboxes2[..., None, :, 3:])

    eps = jittor.array([eps]).to(dtype=union.dtype)
    union = jittor.max(union, eps)
    ious = overlap / union
    if mode in ['iou']:
        return ious
    # calculate gious
    ...
    enclose_area = jittor.max(enclose_area, eps)
