...
from jtmmcv.core.points import (BasePoints, CameraPoints, DepthPoints,
                                 LiDARPoints)
...
class Coord3DMode(IntEnum):
    ...
    def convert_box(box, src, dst, rt_mat=None):
        ...
        Args:
            box (tuple | list | np.ndarray |
                jittor.Var | BaseInstance3DBoxes):
        ...
        Returns:
            (tuple | list | np.ndarray | jittor.Var | BaseInstance3DBoxes): \
        ...
        if single_box:
            ...
            arr = jittor.array(box)[None, :]
        else:
            # avoid modifying the input box
            if is_numpy:
                arr = jittor.array(np.asarray(box)).clone()
        ...
        if src == Coord3DMode.LIDAR and dst == Coord3DMode.CAM:
            if rt_mat is None:
                rt_mat = jittor.Var([[0, -1, 0], [0, 0, -1], [1, 0, 0]]).to(dtype=arr.dtype)
            xyz_size = jittor.concat([y_size, z_size, x_size], dim=-1)
        elif src == Coord3DMode.CAM and dst == Coord3DMode.LIDAR:
            if rt_mat is None:
                rt_mat = jittor.Var([[0, 0, 1], [-1, 0, 0], [0, -1, 0]]).to(dtype=arr.dtype)
            xyz_size = jittor.concat([z_size, x_size, y_size], dim=-1)
        elif src == Coord3DMode.DEPTH and dst == Coord3DMode.CAM:
            if rt_mat is None:
                rt_mat = jittor.Var([[1, 0, 0], [0, 0, 1], [0, -1, 0]]).to(dtype=arr.dtype)
            xyz_size = jittor.concat([x_size, z_size, y_size], dim=-1)
            ...
            if rt_mat is None:
                rt_mat = jittor.Var([[0, -1, 0], [1, 0, 0], [0, 0, 1]]).to(dtype=arr.dtype)
            xyz_size = jittor.concat([y_size, x_size, z_size], dim=-1)
        ...
        if not isinstance(rt_mat, jittor.Var):
            rt_mat = jittor.array(rt_mat).to(dtype=arr.dtype)
        if rt_mat.size(1) == 4:
            extended_xyz = jittor.concat(
        ...
        remains = arr[..., 6:]
        arr = jittor.concat([xyz[:, :3], xyz_size, remains], dim=-1)
    ...
    def convert_point(point, src, dst, rt_mat=None):
        ...
        Returns:
            (tuple | list | np.ndarray | jittor.Var | BasePoints): \
        ...
        if single_point:
            ...
            arr = jittor.array(point)[None, :]
        else:
            # avoid modifying the input point
            if is_numpy:
                arr = jittor.array(np.asarray(point)).clone()
        ...
        if not isinstance(rt_mat, jittor.Var):
            rt_mat = jittor.Var(rt_mat).to(dtype=arr.dtype)
        if rt_mat.size(1) == 4:
            extended_xyz = jittor.concat(
                [arr[:, :3], arr.new_ones(arr.size(0), 1)], dim=-1)
            xyz = extended_xyz @ rt_mat.t()
        ...
        remains = arr[:, 3:]
        arr = jittor.concat([xyz[:, :3], remains], dim=-1)
