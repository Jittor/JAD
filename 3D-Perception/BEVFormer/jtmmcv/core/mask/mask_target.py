...
def mask_target(pos_proposals_list, pos_assigned_gt_inds_list, gt_masks_list,
    ...
    Example:
        >>> import mmcv
        >>> import mmdet
        >>> from jtmmcv.core.mask import BitmapMasks
        >>> from jtmmcv.core.mask.mask_target import *
        >>> H, W = 17, 18
        ...
        >>>     jittor.int32Tensor([7, 0]),
        >>>     jittor.int32Tensor([5, 4, 1]),
        >>> ]
    ...
    if len(mask_targets) > 0:
        mask_targets = jittor.concat(mask_targets)
    return mask_targets
...
def mask_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg):
    ...
    Example:
        ...
        >>> import mmdet
        >>> from jtmmcv.core.mask import BitmapMasks
        >>> from jtmmcv.core.mask.mask_target import *  # NOQA
        >>> H, W = 32, 32
        ...
        >>> pos_assigned_gt_inds = jittor.int32Tensor([0, 1, 2, 1, 1])
    ...
    if num_pos > 0:
        proposals_np = pos_proposals.numpy()
        maxh, maxw = gt_masks.height, gt_masks.width
        proposals_np[:, [0, 2]] = np.clip(proposals_np[:, [0, 2]], 0, maxw)
        proposals_np[:, [1, 3]] = np.clip(proposals_np[:, [1, 3]], 0, maxh)
        pos_assigned_gt_inds = pos_assigned_gt_inds.numpy()
        ...
        mask_targets = jittor.array(mask_targets).float()
    else:
