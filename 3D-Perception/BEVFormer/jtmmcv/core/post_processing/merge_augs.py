...
import numpy as np
import jittor
from jtmmcv import ConfigDict
from jtmmcv.ops import nms
...
def merge_aug_proposals(aug_proposals, img_metas, cfg):
    ...
    aug_proposals = jittor.concat(recovered_proposals, dim=0)
...
def merge_aug_bboxes(aug_bboxes, aug_scores, img_metas, rcnn_test_cfg):
    ...
    bboxes = jittor.stack(recovered_bboxes).mean(dim=0)
    if aug_scores is None:
        return bboxes
    else:
        scores = jittor.stack(aug_scores).mean(dim=0)
        return bboxes, scores


def merge_aug_scores(aug_scores):
    """Merge augmented bbox scores."""
    if isinstance(aug_scores[0], jittor.Var):
        return jittor.mean(jittor.stack(aug_scores), dim=0)
    else:
...
def merge_aug_bboxes_3d(aug_results, img_metas, test_cfg):
    ...
    Args:
        aug_results (list[dict]): The dict of detection results.
            ...
            - scores_3d (jittor.Var): Detection scores.
            - labels_3d (jittor.Var): Predicted box labels.
    ...
    Returns:
        dict: Bounding boxes results in cpu mode, containing merged results.
            ...
            - scores_3d (jittor.Var): Merged detection scores.
            - labels_3d (jittor.Var): Merged predicted box labels.
    """
    ...
    aug_scores = jittor.concat(recovered_scores, dim=0)
    aug_labels = jittor.concat(recovered_labels, dim=0)
    ...
    for class_id in range(jittor.max(aug_labels).item() + 1):
    ...
    merged_scores = jittor.concat(merged_scores, dim=0)
    merged_labels = jittor.concat(merged_labels, dim=0)
