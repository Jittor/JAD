...
from jtmmcv.core.anchor import ANCHOR_GENERATORS
...
class Anchor3DRangeGenerator(object):
    ...
    def num_base_anchors(self):
        ...
        num_size = jittor.array(self.sizes).reshape(-1, 3).size(0)
    ...
    def single_level_grid_anchors(self, featmap_size, scale):
        ...
        Returns:
            jittor.Var: Anchors in the overall feature map.
        """
        # We reimplement the anchor generator using jittor in cuda
        ...
        for anchor_range, anchor_size in zip(self.ranges, self.sizes):
            mr_anchors.append(
                self.anchors_single_range(
                    ...
                    anchor_size,
                    self.rotations))
        mr_anchors = jittor.concat(mr_anchors, dim=-3)
        return mr_anchors
    ...
    def anchors_single_range(self,
        ...
        Args:
            ...
            anchor_range (jittor.Var | list[float]): Range of anchors with
                shape [6]. The order is consistent with that of anchors, i.e.,
                (x_min, y_min, z_min, x_max, y_max, z_max).
            scale (float | int, optional): The scale factor of anchors.
            sizes (list[list] | np.ndarray | jittor.Var): Anchor size with
                shape [N, 3], in order of x, y, z.
            rotations (list[float] | np.ndarray | jittor.Var): Rotations of
                anchors in a single feature grid.
            device (str): Devices that the anchors will be put on.

        Returns:
            jittor.Var: Anchors with shape \
                [*feature_size, num_sizes, num_rots, 7].
        """
        if len(feature_size) == 2:
            feature_size = [1, feature_size[0], feature_size[1]]
        anchor_range = jittor.array(anchor_range)
        z_centers = jittor.linspace(
            anchor_range[2], anchor_range[5], feature_size[0])
        y_centers = jittor.linspace(
            anchor_range[1], anchor_range[4], feature_size[1])
        x_centers = jittor.linspace(
            anchor_range[0], anchor_range[3], feature_size[2])
        sizes = jittor.array(sizes).reshape(-1, 3) * scale
        rotations = jittor.array(rotations)

        # jittor.meshgrid default behavior is 'id', np's default is 'xy'
        rets = jittor.meshgrid(x_centers, y_centers, z_centers, rotations)
        # jittor.meshgrid returns a tuple rather than list
        rets = list(rets)
        tile_shape = [1] * 5
        ...
        ret = jittor.concat(rets, dim=-1).permute([2, 1, 0, 3, 4, 5])
        ...
        if len(self.custom_values) > 0:
            ...
            ret = jittor.concat([ret, custom], dim=-1)
...
class AlignedAnchor3DRangeGenerator(Anchor3DRangeGenerator):
    ...
    def anchors_single_range(self,
        ...
        # jittor.meshgrid default behavior is 'id', np's default is 'xy'
        rets = jittor.meshgrid(x_centers[:feature_size[2]],
...
class AlignedAnchor3DRangeGeneratorPerCls(AlignedAnchor3DRangeGenerator):
    ...
    def grid_anchors(self, featmap_sizes):
        ...
        Returns:
            list[list[jittor.Var]]: Anchors in multiple feature levels. \
        ...
        anchors = self.multi_cls_grid_anchors(
            featmap_sizes, self.scales[0])
        multi_level_anchors.append(anchors)
        return multi_level_anchors

    def multi_cls_grid_anchors(self, featmap_sizes, scale):
