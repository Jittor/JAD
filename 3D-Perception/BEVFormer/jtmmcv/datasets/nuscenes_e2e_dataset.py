...
import numpy as np
import jittor
from jtmmcv.datasets import DATASETS
from jtmmcv.datasets.pipelines import to_tensor
from jtmmcv.datasets import NuScenesDataset
...
from jtmmcv.utils import track_iter_progress, mkdir_or_exist
...
import tempfile
from jtmmcv.parallel.data_container import DataContainer as DC
...
from jtmmcv.datasets.data_utils.vector_map import VectorizedLocalMap
from jtmmcv.datasets.data_utils.rasterize import preprocess_map
from jtmmcv.datasets.eval_utils.map_api import NuScenesMap
from jtmmcv.datasets.data_utils.trajectory_api import NuScenesTraj
...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def prepare_train_data(self, index):
        ...
        if self.filter_empty_gt and \
                (example is None or ~((example['gt_labels_3d']._data != -1).data).any()):
            return None
    ...
    def union2one(self, queue):
        ...
        queue[-1]['img'] = DC(jittor.stack(imgs_list),
    ...
    def get_data_info(self, index):
        ...
        instance_masks = jittor.Var(instance_masks.copy())
        gt_labels = []
        gt_bboxes = []
        gt_masks = []
        for cls in range(self.map_num_classes):
            for i in np.unique(instance_masks[cls]):
                if i == 0:
                    continue
                gt_mask = (instance_masks[cls] == i).to(jittor.uint8)
        ...
        map_mask = jittor.Var(map_mask.copy())
        for i, gt_mask in enumerate(map_mask[:-1]):
            ...
            gt_bboxes.append(gt_bbox)
            gt_masks.append(gt_mask)
        gt_labels = jittor.Var(gt_labels)
        gt_bboxes = jittor.Var(np.stack(gt_bboxes))
        gt_masks = jittor.stack(gt_masks)

        # standard protocal modified from SECOND.
        input_dict = dict(
        ...
        l2g_r_mat = l2e_r_mat.transpose() @ e2g_r_mat.transpose()
        l2g_t = l2e_t @ e2g_r_mat.transpose() + e2g_t
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                lidar2cam_t = cam_info[
                    'sensor2lidar_translation'] @ lidar2cam_r.transpose()
                lidar2cam_rt = np.eye(4)
                lidar2cam_rt[:3, :3] = lidar2cam_r.transpose()
                lidar2cam_rt[3, :3] = -lidar2cam_t
                intrinsic = cam_info['cam_intrinsic']
                viewpad = np.eye(4)
                viewpad[:intrinsic.shape[0], :intrinsic.shape[1]] = intrinsic
                lidar2img_rt = (viewpad @ lidar2cam_rt.transpose())
                lidar2img_rts.append(lidar2img_rt)

                cam_intrinsics.append(viewpad)
                lidar2cam_rts.append(lidar2cam_rt.transpose())
    ...
    def occ_get_transforms(self, indices, data_type=jittor.float32):
        """
        ...
        for index in indices:
            if index == -1:
                ...
                l2e_r_mat = jittor.array(Quaternion(l2e_r).rotation_matrix)
                e2g_r_mat = jittor.array(Quaternion(e2g_r).rotation_matrix)

                l2e_r_mats.append(l2e_r_mat.to(data_type))
                l2e_t_vecs.append(jittor.Var(l2e_t).to(data_type))
                e2g_r_mats.append(e2g_r_mat.to(data_type))
                e2g_t_vecs.append(jittor.Var(e2g_t).to(data_type))
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        #为jittor做出的调整。后续可优化
        for k,v in nusc_submissions['results'].items():
            for i in range(len(v)):
                for j in range(len(v[i]['velocity'])):
                    v[i]['velocity'][j] = float(v[i]['velocity'][j].numpy() ) if isinstance(v[i]['velocity'][j], jittor.Var) else v[i]['velocity'][j]
