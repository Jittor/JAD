        ...
        for layer_name in layer_names:
            for token in records_in_patch[layer_name]:
                ...
                for polygon_token in polygon_tokens:
                    ...
                    points = np.dot(Quaternion(poserecord['rotation']).rotation_matrix.transpose(), points)

                    # Transform into the camera.
                    points = points - np.array(cs_record['translation']).reshape((-1, 1))
                    points = np.dot(Quaternion(cs_record['rotation']).rotation_matrix.transpose(), points)
