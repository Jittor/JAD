...
from jtmmcv.utils import build_from_cfg
from PIL import Image
from jtmmcv.parallel.data_container import DataContainer as DC

from jtmmcv.core import VoxelGenerator
...
from jtmmcv.core.bbox import box_np_ops
from jtmmcv.datasets.builder import PIPELINES
from jtmmcv.datasets.pipelines.transforms import RandomFlip
from jtmmcv.image import impad, impad_to_multiple, imnormalize, imresize, bgr2hsv, hsv2bgr
...
class GlobalAlignment(object):
    ...
    def _rot_points(self, input_dict, rot_mat):
        ...
        input_dict['points'].rotate(rot_mat.transpose())
...
class GlobalRotScaleTrans(object):
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        trans_factor = np.random.normal(scale=translation_std, size=3).transpose()
...
class ObjectRangeFilter(object):
    ...
    def __call__(self, input_dict):
        ...
        # mask is a jittor tensor but gt_labels_3d is still numpy array
...
class PointSample(object):
    ...
    def __call__(self, results):
        ...
        if self.sample_range is not None:
            from jtmmcv.core.points import CameraPoints
...
class CropResizeFlipImage(object):
    ...
    def __call__(self, results):
        ...
        for i in range(N):
            ...
            results['cam_intrinsic'][i][:3, :3] = np.matmul(ida_mat, results['cam_intrinsic'][i][:3, :3])

            if self.debug:
                pts_to_img_post_aug = np.matmul(results['cam_intrinsic'][i], results['lidar2cam'][i]) @ lidar_pts
            ...
            if 'mono_ann_idx' in results.keys():
                ...
                if i in results['mono_ann_idx'].data:
                    ...
                    if isinstance(intrinsics,jittor.Var):
        ...
        results["img"] = new_imgs
        results['lidar2img'] = [np.matmul(results['cam_intrinsic'][i], results['lidar2cam'][i]) for i in
