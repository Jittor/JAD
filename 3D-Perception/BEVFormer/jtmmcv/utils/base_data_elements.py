...
class BaseDataElement:
    ...
    the ``metainfo`` and the ``data`` respectively.
        ...
        - ``data``: Annotations or model predictions are
          ...
          methods to all :obj:`jittor.Var` in the ``data_fields``,
          such as ``.cuda()``, ````, ``.numpy()``, ``.to()``,
          ``to_tensor()``, ``.detach()``.
    ...
    Examples:
        ...
        >>> bboxes = jittor.rand((5, 4))
        >>> scores = jittor.rand((5,))
        >>> img_id = 0
        >>> img_shape = (800, 1333)
        >>> gt_instances = BaseDataElement(
        ...
        (100, 100)
        >>> gt_instances.scores = jittor.rand((5,))
        ...
        >>> gt_instances.bboxes = jittor.rand((5, 4))
        ...
        ...     bboxes=jittor.rand((6, 4)), scores=jittor.rand((6,)))
        ...
        >>> cpu_instances = cuda_instances
        >>> cpu_instances = cuda_instances.to('cpu')
        >>> fp16_instances = cuda_instances.to(
        ...     device=None, dtype=jittor.float16, non_blocking=False,
        ...     copy=False, memory_format=jittor.preserve_format)
        ...
        >>> gt_instances = BaseDataElement(
        ...     metainfo=metainfo, det_labels=jittor.int32Tensor([0, 1, 2, 3]))
        ...
        >>> proposals = BaseDataElement(bboxes=jittor.rand((5, 4)))
        ...
        ...     det_sample.proposals = jittor.rand((5, 4))
    """
    ...
    def cpu(self) -> 'BaseDataElement':
        ...
        new_data = self.new()
        for k, v in self.items():
            if isinstance(v, (jittor.Var, BaseDataElement)):
                v = v
                data = {k: v}
                new_data.set_data(data)
        return new_data
