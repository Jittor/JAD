...
def cast_tensor_type(inputs, src_type, dst_type):
    ...
    Args:
        ...
        src_type (jittor.dtype): Source type..
        dst_type (jittor.dtype): Destination type.
    ...
    if isinstance(inputs, Module):
        return inputs
    elif isinstance(inputs, jittor.Var):
...
def auto_fp16(apply_to=None, out_fp32=False):
    ...
    ignored. If you are using PyTorch >= 1.6, jittor.cuda.amp is used as the
    ...
    def auto_fp16_wrapper(old_func):
        ...
        def new_func(*args, **kwargs):
            ...
            if args:
                ...
                for i, arg_name in enumerate(arg_names):
                    if arg_name in args_to_cast:
                        new_args.append(
                            cast_tensor_type(args[i], jittor.float32, jittor.float16))
                    else:
                ...
                for arg_name, arg_value in kwargs.items():
                    if arg_name in args_to_cast:
                        new_kwargs[arg_name] = cast_tensor_type(
                            arg_value, jittor.float32, jittor.float16)
                    else:
            ...
            if out_fp32:
                output = cast_tensor_type(output, jittor.float16, jittor.float32)
            return output
        ...
        def new_func(*args, **kwargs):
            ...
            if args:
                ...
                for i, arg_name in enumerate(arg_names):
                    if arg_name in args_to_cast:
                        new_args.append(
                            cast_tensor_type(args[i], jittor.half, jittor.float32))
                    else:
                        new_args.append(args[i])
            # convert the kwargs that need to be processed
            new_kwargs = dict()
            if kwargs:
                for arg_name, arg_value in kwargs.items():
                    if arg_name in args_to_cast:
                        new_kwargs[arg_name] = cast_tensor_type(
                            arg_value, jittor.half, jittor.float32)
                    else:
                        new_kwargs[arg_name] = arg_value
            # apply converted arguments to the decorated method
            output = old_func(*new_args, **new_kwargs)
            # cast the results back to fp32 if necessary
            if out_fp16:
                output = cast_tensor_type(output, jittor.float32, jittor.half)
            return output
...
def patch_norm_fp32(module):
    ...
    Args:
        module (Module): The modules to be converted in FP16.

    Returns:
        nn.Module: The converted module, the normalization layers have been
            converted to FP32.
    """
    if isinstance(module, (jnn.BatchNorm, jnn.GroupNorm)):
        module.float()
        if isinstance(module, jnn.GroupNorm):
            module.execute = patch_forward_method(module.execute, jittor.half,
                                                  jittor.float32)
...
def patch_forward_method(func, src_type, dst_type, convert_output=True):
    ...
    Args:
        ...
        src_type (jittor.dtype): Type of input arguments to be converted from.
        dst_type (jittor.dtype): Type of input arguments to be converted to.
