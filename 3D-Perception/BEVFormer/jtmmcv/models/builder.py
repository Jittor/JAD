...
# from jtmmcv.cnn import MODELS as MMCV_MODELS
...
def build_model_from_cfg(cfg, registry, default_args=None):
    """Build a jittor model from config dict(s). Different from
    ``build_from_cfg``, if cfg is a list, a ``jnn.Sequential`` will be built.

    Args:
        cfg (dict, list[dict]): The config of modules, is is either a config
            dict or a list of config dicts. If cfg is a list, a
            the built modules will be wrapped with ``jnn.Sequential``.
