from jtmmcv.core import mask
import jittor
from jtmmcv.core.bbox.assigners.base_assigner import BaseAssigner

from jtmmcv.core.bbox.assigners.assign_result import AssignResult
from jtmmcv.core.bbox.transforms import bbox_cxcywh_to_xyxy
from jtmmcv.core.bbox.match_costs import build_match_cost
from jtmmcv.core.bbox.builder import BBOX_ASSIGNERS
try:
    from scipy.optimize import linear_sum_assignment
except ImportError:
    linear_sum_assignment = None

from jtmmcv.core.bbox.samplers.base_sampler import BaseSampler
from jtmmcv.core.bbox.builder import BBOX_SAMPLERS
from jtmmcv.core import mask

from jtmmcv.utils import util_mixins
...
class SamplingResult_segformer(util_mixins.NiceRepr):
    ...
    @property
    def bboxes(self):
        """jittor.Var: concatenated positive and negative boxes"""
        return jittor.concat([self.pos_bboxes, self.neg_bboxes])
    ...
    def to(self, device):
        ...
        for key, value in _dict.items():
            if isinstance(value, jittor.Var):
    ...
    def random(cls, rng=None, **kwargs):
        ...
        Example:
            >>> from jtmmcv.core.bbox.samplers.sampling_result import *  # NOQA
            >>> self = SamplingResult.random() 
            >>> print(self.__dict__)
        """
        # 后面看一下会不会调用 源码没掉用
        from jtmmcv.core.bbox.samplers.random_sampler import RandomSampler
        from jtmmcv.core.bbox.assigners.assign_result import AssignResult
        from jtmmcv.core.bbox import demodata
...
class PseudoSampler_segformer(BaseSampler):
    ...
    def sample(self, assign_result, bboxes, gt_bboxes,gt_masks, **kwargs):
        ...
        neg_inds = jittor.nonzero(
            assign_result.gt_inds == 0).squeeze(-1).unique()
        gt_flags = bboxes.new_zeros(bboxes.shape[0]).to(jittor.int8)
...
class HungarianAssigner_filter(BaseAssigner):
    ...
    def assign(self,
        ...
        assigned_labels = bbox_pred.new_full((num_bboxes, ),-1,)
        ...
        factor = gt_bboxes.new([img_w, img_h, img_w,
                                       img_h]).unsqueeze(0)
        ...
        for i in range(min(self.max_pos, 300//num_gts)):
            matched_row_inds, matched_col_inds = linear_sum_assignment(cost.data)
            
            matched_row_inds = jittor.array(matched_row_inds)
            matched_col_inds = jittor.array(matched_col_inds)
            ...
            if cost[matched_row_inds, matched_col_inds].max()>=INF:
                break
        pos_ind = (assigned_gt_inds>0).nonzero().squeeze(1)
        neg_ind = (assigned_gt_inds==0).nonzero().squeeze(1)
...
class HungarianAssigner_multi_info(BaseAssigner):
    ...
    def assign(self,
        ...
        factor = bbox_pred.new([img_w, img_h, img_w,img_h]).unsqueeze(0)
