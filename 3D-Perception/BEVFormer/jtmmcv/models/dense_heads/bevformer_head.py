...
class BEVFormerHead(DETRHead):
    ...
    def execute(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        """execute function.
        Args:
            mlvl_feats (tuple[jittor.Var]): Features from the upstream
                network, each is a 5D-jittor.Var with shape
                (B, N, C, H, W).
            prev_bev: previous bev featues
            only_bev: only compute BEV features with encoder. 
        Returns:
            all_cls_scores (jittor.Var): Outputs from the classification head, \
                shape [nb_dec, bs, num_query, cls_out_channels]. Note \
                cls_out_channels should includes background.
            all_bbox_preds (jittor.Var): Sigmoid outputs from the regression \
