...
# from jtmmcv.ops import DeformConv2d, MaskedConv2d
from jtmmcv.ops.deform_conv2 import DCN as DeformConv2d
from jtmmcv.ops.masked_conv import MaskedConv2d
from jtmmcv.models.backbones import BaseModule
from jtmmcv.utils import force_fp32

from jtmmcv.core.bbox import build_bbox_coder
from jtmmcv.core.anchor import anchor_inside_flags, build_anchor_generator, images_to_levels, calc_region
from jtmmcv.core.utils import multi_apply, reduce_mean, unmap
from jtmmcv.core.bbox.builder import build_assigner, build_sampler
from jtmmcv.core.post_processing.bbox_nms import multiclass_nms
...
class GuidedAnchorHead(AnchorHead):
    ...
    def _init_layers(self):
        self.relu = jnn.ReLU()
        self.conv_loc = jnn.Conv2d(self.in_channels, 1, 1)
        self.conv_shape = jnn.Conv2d(self.in_channels, self.num_anchors * 2, 1)
    ...
    def get_sampled_approxs(self, featmap_sizes, img_metas, device='cuda'):
        ...
        Args:
            ...
            device (jittor.device | str): device for returned tensors
        ...
        for img_id, img_meta in enumerate(img_metas):
            ...
            for i, flags in enumerate(multi_level_approx_flags):
                ...
                # position is true
                inside_flags = (
                    jittor.stack(inside_flags_list, 0).sum(dim=0) > 0)
    ...
    def ga_loc_targets(self, gt_bboxes_list, featmap_sizes):
        ...
        for lvl_id in range(num_lvls):
            ...
            loc_targets = jittor.zeros(
                (img_per_gpu,
                1,
                h,
                w),
                dtype=jittor.float32)
            loc_weights = jittor.full_like(loc_targets, -1)
            ignore_map = jittor.zeros_like(loc_targets)
            all_loc_targets.append(loc_targets)
            all_loc_weights.append(loc_weights)
            all_ignore_map.append(ignore_map)
        for img_id in range(img_per_gpu):
            gt_bboxes = gt_bboxes_list[img_id]
            scale = jittor.sqrt((gt_bboxes[:, 2] - gt_bboxes[:, 0]) *
                               (gt_bboxes[:, 3] - gt_bboxes[:, 1]))
            min_anchor_size = scale.new_full(
                (1, ), float(anchor_scale * anchor_strides[0]))
            # assign gt bboxes to different feature levels w.r.t. their scales
            target_lvls = jittor.floor(
                jittor.log2(scale) - jittor.log2(min_anchor_size) + 0.5)
            target_lvls = target_lvls.clamp(min_v=0, max_v=num_lvls - 1).long()
    ...
    def _ga_shape_target_single(self,
        ...
        bbox_anchors = jittor.zeros_like(squares)
        bbox_gts = jittor.zeros_like(squares)
        bbox_weights = jittor.zeros_like(squares)
    ...
    def ga_shape_targets(self,
        ...
        for i in range(num_imgs):
            ...
            inside_flag_flat_list.append(jittor.concat(inside_flag_list[i]))
            approx_flat_list.append(jittor.concat(approx_list[i]))
            square_flat_list.append(jittor.concat(square_list[i]))
    ...
    def loss(self,
        ...
        for i in range(len(guided_anchors_list)):
            concat_anchor_list.append(jittor.concat(guided_anchors_list[i]))
    ...
    def _get_bboxes_single(self,
        ...
        for cls_score, bbox_pred, anchors, mask in zip(cls_scores, bbox_preds,
            ...
            if nms_pre > 0 and scores.shape[0] > nms_pre:
                if self.use_sigmoid_cls:
                     _,max_scores = scores.argmax(dim=1)
                else:
                    # remind that we set FG labels to [0, num_class-1]
                    # since mmcv v2.0
                    # BG cat_id: num_class
                    _,max_scores = scores[:, :-1].argmax(dim=1)
        ...
        mlvl_bboxes = jittor.concat(mlvl_bboxes)
        if rescale:
            mlvl_bboxes /= mlvl_bboxes.new(scale_factor)
        mlvl_scores = jittor.concat(mlvl_scores)
        if self.use_sigmoid_cls:
            # Add a dummy background class to the backend when using sigmoid
            # remind that we set FG labels to [0, num_class-1] since mmcv v2.0
            # BG cat_id: num_class
            padding = mlvl_scores.new_zeros(mlvl_scores.shape[0], 1)
            mlvl_scores = jittor.concat([mlvl_scores, padding], dim=1)
        # multi class NMS
