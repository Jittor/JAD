...
from jtmmcv.models.detectors.mvx_two_stage import MVXTwoStageDetector
from jtmmcv.models.utils.grid_mask import GridMask
import copy
import math
...
class UniADTrack(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        self.query_embedding = jnn.Embedding(self.num_query+1, self.embed_dims * 2)   # the final one is ego query, which constantly models ego-vehicle
        self.reference_points = jnn.Linear(self.embed_dims, 3)
    ...
    def _generate_empty_tracks(self):
        ...
        pred_boxes_init = jittor.zeros(
            (len(track_instances), 10), dtype=jittor.float32
        )
        track_instances.query = query

        track_instances.output_embedding = jittor.zeros(
            (num_queries, dim >> 1)
        )

        track_instances.obj_idxes = jittor.full(
            (len(track_instances),), -1, dtype=jittor.int32
        )
        track_instances.matched_gt_idxes = jittor.full(
            (len(track_instances),), -1, dtype=jittor.int32
        )
        track_instances.disappear_time = jittor.zeros(
            (len(track_instances),), dtype=jittor.int32
        )

        track_instances.iou = jittor.zeros(
            (len(track_instances),), dtype=jittor.float32
        )
        track_instances.scores = jittor.zeros(
            (len(track_instances),), dtype=jittor.float32
        )
        track_instances.track_scores = jittor.zeros(
            (len(track_instances),), dtype=jittor.float32
        )
        # xy, wl, z, h, sin, cos, vx, vy, vz
        track_instances.pred_boxes = pred_boxes_init

        track_instances.pred_logits = jittor.zeros(
            (len(track_instances), self.num_classes), dtype=jittor.float32
        )

        mem_bank_len = self.mem_bank_len
        track_instances.mem_bank = jittor.zeros(
            (len(track_instances), mem_bank_len, dim // 2),
            dtype=jittor.float32,
        )
        track_instances.mem_padding_mask = jittor.ones(
            (len(track_instances), mem_bank_len), dtype=bool
        )
        track_instances.save_period = jittor.zeros(
            (len(track_instances),), dtype=jittor.float32
        )
    ...
    def velo_update(
        ...
        time_delta = time_delta.to(jittor.float32)
        num_query = ref_pts.size(0)
        velo_pad_ = velocity.new_zeros((num_query, 1))
        velo_pad = jittor.concat((velocity, velo_pad_), dim=-1)
        ...
        g2l_r = jittor.linalg.inv(l2g_r2).to(jittor.float32)
    ...
    def _copy_tracks_for_loss(self, tgt_instances):
        ...
        track_instances.pred_boxes = jittor.zeros(
            (len(track_instances), 10), dtype=jittor.float32
        )
        track_instances.iou = jittor.zeros(
            (len(track_instances),), dtype=jittor.float32
    ...
    def _forward_single_frame_train(
        ...
        with jittor.no_grad():
            track_scores = output_classes[-1, 0, :].sigmoid().max(dim=-1)
    ...
    def forward_track_train(self,
        ...
        for i in range(num_frame):
            ...
            gt_instances.sdc_boxes = jittor.concat([sd_boxes for _ in range(boxes.shape[0])], dim=0)  # boxes.shape[0] sometimes 0
            gt_instances.sdc_labels = jittor.concat([gt_sdc_label[0][i] for _ in range(gt_labels_3d[0][i].shape[0])], dim=0)
        ...
        for i in range(num_frame):
            ...
            img_single = jittor.stack([img_[i] for img_ in img], dim=0)
    ...
    def upsample_bev_if_tiny(self, outs_track):
        if outs_track["bev_embed"].size(0) == 100 * 100:
            ...
            bev_embed = jnn.Upsample(scale_factor=2)(bev_embed)  # [1, 256, 200, 200]
            ...
            if prev_bev is not None:
                if self.training:
                    ...
                    prev_bev = jnn.Upsample(scale_factor=2)(prev_bev)  # [1, 256, 200, 200]
            ...
            bev_pos = jnn.Upsample(scale_factor=2)(bev_pos)  # [1, 256, 200, 200]
    ...
    def _det_instances2results(self, instances, results, img_metas):
        ...
        - 'obj_idxes'
        out_dict. keys:
            - boxes_3d (jittor.Var): 3D boxes.
            - scores (jittor.Var): Prediction scores.
            - labels_3d (jittor.Var): Box labels.
            - attrs_3d (jittor.Var, optional): Box attributes.
            - track_ids
            - tracking_score
        """
